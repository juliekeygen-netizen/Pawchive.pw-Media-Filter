...
(function () {
  ...
  const Config = Object.freeze({
    ...
    popularAggregatePeriodKey: 'pmf-popular-aggregate-period-v1',
    popularQueueSessionKey: 'pmf-popular-queue-session-v1',
    popularQueueOpenKey: 'pmf-popular-queue-open-v1',
  ...
  const Settings = {
    ...
    normalize(source = {}) {
      ...
      output.synchronizeNativeFavorites=typeof safe.synchronizeNativeFavorites==='boolean'
        ...
        : true;
      output.confirmCreatorCardScan=false;
  ...
  const Route = {
    ...
    canonicalPopularUrl(url=location.href){
      let parsed;try{parsed=new URL(url,location.origin);}catch{return'';}if(!Config.allowedHosts.has(parsed.hostname.toLowerCase()))return'';const segments=parsed.pathname.split('/').filter(Boolean);if(segments.length!==2||segments[0].toLowerCase()!=='posts'||segments[1].toLowerCase()!=='popular')return'';const rawPeriod=String(parsed.searchParams.get('period')||'').toLowerCase(),rawDate=String(parsed.searchParams.get('date')||'');const periodValid=['day','week','month'].includes(rawPeriod),dateValid=/^\d{4}-\d{2}-\d{2}$/.test(rawDate)&&new Date(`${rawDate}T00:00:00Z`).toISOString().slice(0,10)===rawDate;if(periodValid&&dateValid)return'';parsed.searchParams.set('period',periodValid?rawPeriod:'day');parsed.searchParams.set('date',dateValid?rawDate:new Date().toISOString().slice(0,10));return parsed.href;
    },
  ...
  const Cache = {
    ...
    async getPopularEntriesForPeriods(periodKeys=[]) {
      const keys=[...new Set((periodKeys||[]).map(String).filter(Boolean))];if(!keys.length)return[];
      const wanted=new Set(keys),db=await Cache.open();if(!db)return[...Cache.popularEntryMemory.values()].filter((entry)=>wanted.has(String(entry.periodKey||''))).map(Util.clone);
      return new Promise((resolve,reject)=>{const tx=db.transaction('popularEntries','readonly'),index=tx.objectStore('popularEntries').index('periodKey'),records=[];let pending=keys.length;for(const key of keys){const request=index.getAll(key);request.onsuccess=()=>{records.push(...(request.result||[]));pending-=1;if(!pending)resolve(records);};request.onerror=()=>reject(request.error);}tx.onabort=()=>reject(tx.error||new Error('Popular aggregate entry lookup transaction aborted'));}).catch((error)=>{Logger.warn('Could not read aggregate Popular entries.',error);return[];});
    },
  ...
  const PostStatusFilters = {
    ...
    normalize(value={}){return Object.fromEntries(['favorite','liked','seen'].map((field)=>{let mode=PostStatusFilters.states.has(value?.[field])?value[field]:'off';if(field==='seen'&&mode==='match')mode='no-match';return[field,mode];}));},
    ...
    cycle(field){const order=field==='seen'?['off','no-match']:['off','match','no-match'];const current=PostStatusFilters.value[field]||'off';PostStatusFilters.value[field]=order[(order.indexOf(current)+1)%order.length];return PostStatusFilters.save();},
  };
  ...
  const FilterSummary = {
    names(state) {
      ...
      if (normalized.publishedDate.enabled) active.push('Date published');
      return active;
    },
    label(state) {
  ...
  const CreatorStatusFilters = {
    ...
    normalize(value={}){return Object.fromEntries(CreatorStatusFilters.fields.map((field)=>{let mode=CreatorStatusFilters.values.includes(value?.[field])?value[field]:'off';if(field==='hidden'&&mode==='match')mode='no-match';return[field,mode];}));},
    ...
    cycle(value,field=''){if(field==='hidden')return value==='off'?'no-match':'off';return{off:'match',match:'no-match','no-match':'off'}[value]||'match';},
  ...
  const CreatorPresets = {
    ...
    duplicate(record,id){const next=CreatorPresets.normalize(record),source=next.presets.find((item)=>item.id===id);if(!source)return{valid:false,message:'That creator preset no longer exists.',record:next,preset:null};let name=`${source.name} copy`,suffix=2;while(next.presets.some((item)=>item.name.toLocaleLowerCase()===name.toLocaleLowerCase()))name=`${source.name} copy ${suffix++}`;const preset={id:`creator-preset-${Date.now()}-${Math.random().toString(36).slice(2)}`,name,state:CreatorFilterEngine.normalizeState(source.state)};next.presets.push(preset);next.activeId=preset.id;return{valid:true,message:'',record:CreatorPresets.save(next),preset};},
  ...
  const CreatorFilterEngine = {
    ...
    normalizeState(value={}) {
      ...
      return {
        ...
        media,
        customRulesEnabled:requestedRulesEnabled,
        customRules,
      };
    },
  ...
  const CreatorCardReconstructor = {
    ...
    buildFromTemplate(record){
      if(!CreatorCardReconstructor.template)return null;const directory=record.directory||{};const card=CreatorCardReconstructor.sanitize(CreatorCardReconstructor.template.cloneNode(true));card.dataset.pmfOwned='true';card.classList.add('pmf-catalogue-creator-card');card.dataset.pmfCreatorService=['fanbox','pixivfanbox'].includes(String(directory.service||'').toLowerCase())?'fanbox':'patreon';const link=card.matches('a[href]')?card:card.querySelector('a[href]');if(!link)return null;link.href=CreatorCardReconstructor.safeUrl(directory.creatorUrl)||'#';
      ...
      const service=card.querySelector('.user-card__service,[class*="service"]');if(service){service.textContent=directory.serviceLabel||CreatorDisplayName.serviceLabel(directory.service);service.dataset.pmfCreatorServiceBadge=card.dataset.pmfCreatorService;}const name=card.querySelector('.user-card__name,[class*="name"]');if(name)name.textContent=directory.creatorName||directory.creatorId||'Creator';const count=card.querySelector('.user-card__count,[class*="count"]');if(count)count.innerHTML=directory.publicFavoriteCount==null?'':`<b>${Number(directory.publicFavoriteCount).toLocaleString()}</b> favorites`;const content=card.querySelector('.user-card__info,[class*="info"]');content?.setAttribute('data-pmf-creator-content','true');return{card,link};
    },
    build(record){
      const templated=CreatorCardReconstructor.buildFromTemplate(record);if(templated)return templated;
      const directory=record.directory||{};const card=document.createElement('article');card.className='pmf-reconstructed-creator-card';card.dataset.pmfOwned='true';card.dataset.pmfCreatorService=['fanbox','pixivfanbox'].includes(String(directory.service||'').toLowerCase())?'fanbox':'patreon';
      ...
      const content=document.createElement('span');content.className='pmf-reconstructed-creator-content';content.dataset.pmfCreatorContent='true';const service=document.createElement('small');service.textContent=directory.serviceLabel||CreatorDisplayName.serviceLabel(directory.service);service.dataset.pmfCreatorServiceBadge=card.dataset.pmfCreatorService;const name=document.createElement('strong');name.textContent=directory.creatorName||directory.creatorId||'Creator';content.append(service,name);
  ...
  const NativePaginatorMirror = {
    ...
    displayLabel(role,label=''){if(role==='first')return'«';if(role==='previous')return'‹';if(role==='next')return'›';if(role==='last')return'»';if(role.startsWith('page:'))return role.slice(5);return String(label||'').trim();},
    items(native){
      const scope=native?.querySelector?.('menu')||native;const selector='a[href],a[data-value],button,li';const nodes=[...scope?.querySelectorAll?.(selector)||[]];
      const candidates=nodes.map((item,index)=>{const label=String(item.textContent||item.getAttribute?.('aria-label')||'').trim(),role=NativePaginatorMirror.role(label,index);return{item,index,label,role,anchor:item.matches?.('a[href],a[data-value],button')||false,disabled:item.matches?.('li,.pagination-button-disabled,[disabled],[aria-disabled="true"]')||false,current:item.matches?.('.pagination-button-current,[aria-current="page"]')||false};}).filter((candidate)=>!candidate.role.startsWith('other:'));
    ...
    limitPageItems(items,limit=Infinity,{pendingRole=''}={}){const maximum=Number(limit);if(!Number.isFinite(maximum)||maximum<1)return items;const pages=items.filter((item)=>item.role.startsWith('page:')).sort((a,b)=>Number(a.role.slice(5))-Number(b.role.slice(5)));if(pages.length<=maximum)return items;const current=pages.find((item)=>item.current)||pages.find((item)=>item.role===pendingRole)||pages[0];const currentNumber=Number(current?.role.slice(5))||1;const pageNumbers=Paginator.pageButtons(currentNumber,Math.max(...pages.map((item)=>Number(item.role.slice(5))||1)),maximum);const allowed=new Set(pageNumbers.map((page)=>`page:${page}`));return items.filter((item)=>!item.role.startsWith('page:')||allowed.has(item.role));},
    compactItems(items,{pendingRole='',enabled=true}={}){
      const mobile=enabled&&globalThis.matchMedia?.('(max-width: 760px)')?.matches;
    ...
    render(native,host,{includeCount=true,countText='',pendingRole='',navigationPending=false,omitFirstLast=false,maxPageButtons=Infinity,compactOnMobile=true}={}){
      host?.replaceChildren?.();if(!native||!host)return false;
      let items=NativePaginatorMirror.items(native);if(omitFirstLast)items=items.filter((item)=>!['first','last'].includes(item.role));items=NativePaginatorMirror.limitPageItems(items,maxPageButtons,{pendingRole});items=NativePaginatorMirror.compactItems(items,{pendingRole,enabled:compactOnMobile});if(!items.length)return false;
      if(includeCount){const nativeCount=native.querySelector('small'),value=String(countText||nativeCount?.textContent||'').trim();if(value){const count=document.createElement('small');count.className='pmf-filtered-count';count.textContent=value;host.append(count);}}
      const menu=document.createElement('div');menu.className='pmf-page-controls';items.forEach(({index,label,role,disabled,current})=>{const button=document.createElement('button');button.type='button';button.textContent=NativePaginatorMirror.displayLabel(role,label);button.dataset.nativePaginatorIndex=String(index);button.dataset.nativePaginatorRole=role;button.disabled=disabled||navigationPending;if(['first','previous','next','last'].includes(role))button.classList.add('pmf-page-nav');if(current||pendingRole&&role===pendingRole){button.setAttribute('aria-current','page');button.classList.add('pmf-current-page');}if(pendingRole&&role===pendingRole){button.classList.add('pmf-pending-page');button.setAttribute('aria-busy','true');}menu.append(button);});host.append(menu);return true;
    },
  ...
  const PopularNavigation = {
    ...
    visit(value,{reason='popular-navigation'}={}){
      ...
      if(App.pageKind==='popular'&&['native','local','aggregate'].includes(App.popularMode)){GM_setValue(Config.popularModeKey,App.popularMode);GM_setValue(Config.popularAggregatePeriodKey,PopularPeriod.normalizePeriod(App.popularAggregatePeriod));}
  ...
  const PopularDatePicker = {
    label(period='day'){const value=PopularPeriod.normalizePeriod(period);return value[0].toUpperCase()+value.slice(1);},
    help(period='day'){const value=PopularPeriod.normalizePeriod(period);return value==='day'?'Choose the exact day to open.':value==='week'?'Choose any day inside the week you want to open.':'Choose any day inside the month you want to open.';},
    open(period='day',opener=document.activeElement){
      const normalized=PopularPeriod.normalizePeriod(period),label=PopularDatePicker.label(normalized),initial=PopularPeriod.normalizeDate(App.popularContext?.date)||PopularPeriod.todayUtc();
      const backdrop=document.createElement('div');backdrop.className='pmf-modal-backdrop';backdrop.innerHTML=`<section class="pmf-dialog pmf-small-dialog pmf-surface pmf-popular-date-dialog" role="dialog" aria-modal="true" aria-labelledby="pmf-popular-date-title"><header><strong id="pmf-popular-date-title">Open custom ${label}</strong><button type="button" class="pmf-icon-close" data-close aria-label="Close">×</button></header><div class="pmf-editor-body"><label class="pmf-block-label">Date<input type="date" name="popularDate" value="${Util.escapeHtml(initial)}"></label><p class="pmf-help">${Util.escapeHtml(PopularDatePicker.help(normalized))}</p><div class="pmf-editor-error" aria-live="polite"></div></div><footer><button type="button" data-close>Cancel</button><button type="button" class="pmf-primary" data-go>Go</button></footer></section>`;
      const dialog=backdrop.firstElementChild,input=dialog.querySelector('[name="popularDate"]'),error=dialog.querySelector('.pmf-editor-error');GlobalUI.ensureHost().append(backdrop);UI.own(backdrop);const overlay=OverlayManager.open({node:dialog,root:backdrop,modal:true,opener});
      const go=()=>{const date=PopularPeriod.normalizeDate(input.value);if(!date){error.textContent='Choose a valid date.';input.focus();return false;}const url=new URL('/posts/popular',location.origin);url.searchParams.set('date',date);url.searchParams.set('period',normalized);const nextMode=App.popularMode==='aggregate'?'native':App.popularMode;App.popularMode=nextMode;GM_setValue(Config.popularModeKey,nextMode);OverlayManager.close(overlay,'go');return PopularNavigation.visit(url.href,{reason:'popular-custom-date'});};
      backdrop.addEventListener('click',(event)=>{if(event.target.closest('[data-close]'))OverlayManager.close(overlay,'cancel');if(event.target.closest('[data-go]'))go();});input.addEventListener('keydown',(event)=>{if(event.key==='Enter'){event.preventDefault();go();}});queueMicrotask(()=>input.focus());return overlay;
    },
  };

  const PopularDOM = {
    isOwned(node){return Boolean(node?.closest?.('[data-pmf-owned="true"]'));},
    loggedIn(root=document){return[...root.querySelectorAll?.('a[href],button,form')||[]].some((node)=>{const text=String(node.textContent||node.getAttribute?.('aria-label')||'').trim(),href=String(node.getAttribute?.('href')||node.getAttribute?.('action')||'');return /\blog\s*out\b/i.test(text)||/\/(?:account\/)?logout(?:[/?#]|$)/i.test(href);});},
    restrictionNoticeCandidates(root=document,{main=null,grid=null}={}){if(!PopularDOM.loggedIn(root))return[];const scope=main||root,phrases=[/only available to registered users/i,/visit the login page/i,/registration page/i],textOf=(node)=>String(node?.textContent||'').replace(/\s+/g,' ').trim(),safe=(node)=>node&&!PopularDOM.isOwned(node)&&node!==grid&&!node.contains?.(grid)&&!node.querySelector?.('article.post-card,.post-card,#pmf-popular-root'),all=(text)=>phrases.every((pattern)=>pattern.test(text)),elements=[...scope.querySelectorAll?.('p,div,section,aside')||[]].filter(safe),groups=elements.filter((node)=>all(textOf(node))).filter((node)=>![...node.children||[]].some((child)=>all(textOf(child))));if(groups.length)return groups;return elements.filter((node)=>phrases.some((pattern)=>pattern.test(textOf(node)))&&![...node.children||[]].some((child)=>phrases.some((pattern)=>pattern.test(textOf(child)))));},
    ...
    find(page={}){
      ...
      const restrictionNotices=PopularDOM.restrictionNoticeCandidates(document,{main,grid});
      ...
      return{main,grid,nativeCards,template:nativeCards[0]||null,paginators,countNodes,topPaginator:paginators[0]||null,bottomPaginator:paginators.at(-1)||null,heading,periodLinks,navContainer,navContainers,restrictionNotices,totalPosts};
    },
  ...
  const PopularAggregate = {
    favorite(entry={}){const raw=entry.displayedFavoriteCount;if(raw==null||raw==='')return Number.NEGATIVE_INFINITY;const value=Number(raw);return Number.isFinite(value)?value:Number.NEGATIVE_INFINITY;},
    isBetter(candidate={},current=null){if(!current)return true;const nextFavorite=PopularAggregate.favorite(candidate),currentFavorite=PopularAggregate.favorite(current);if(nextFavorite!==currentFavorite)return nextFavorite>currentFavorite;const nextObserved=Math.max(0,Number(candidate.observedAt)||0),currentObserved=Math.max(0,Number(current.observedAt)||0);if(nextObserved!==currentObserved)return nextObserved>currentObserved;const nextRank=Math.max(0,Number(candidate.rank)||0),currentRank=Math.max(0,Number(current.rank)||0);if(Boolean(nextRank)!==Boolean(currentRank))return Boolean(nextRank);return nextRank&&currentRank?nextRank<currentRank:false;},
    collapse(entries=[]){const best=new Map();for(const raw of entries||[]){const entry=raw&&typeof raw==='object'?raw:null,postKey=String(entry?.postKey||'');if(!postKey)continue;const current=best.get(postKey);if(PopularAggregate.isBetter(entry,current))best.set(postKey,{...entry,sourcePeriodKey:String(entry.periodKey||''),aggregate:true});}const ordered=[...best.values()].sort((a,b)=>{const aFavorite=PopularAggregate.favorite(a),bFavorite=PopularAggregate.favorite(b);if(aFavorite!==bFavorite)return bFavorite>aFavorite?1:-1;const observedDifference=(Number(b.observedAt)||0)-(Number(a.observedAt)||0);if(observedDifference)return observedDifference;return String(a.postKey||'').localeCompare(String(b.postKey||''));});return ordered.map((entry,index)=>({...entry,rank:index+1}));},
    async load({domain=location.hostname,period='day'}={}){const normalizedPeriod=PopularPeriod.normalizePeriod(period),normalizedDomain=String(domain||location.hostname).toLowerCase(),periods=(await Cache.getPopularPeriods()).filter((item)=>String(item?.domain||'').toLowerCase()===normalizedDomain&&PopularPeriod.normalizePeriod(item?.period)===normalizedPeriod);const sourceEntries=await Cache.getPopularEntriesForPeriods(periods.map((item)=>item.periodKey)),contributingKeys=new Set(sourceEntries.map((entry)=>String(entry.periodKey||'')).filter(Boolean)),contributingPeriods=periods.filter((item)=>contributingKeys.has(String(item.periodKey||''))),entries=PopularAggregate.collapse(sourceEntries),posts=await Cache.getPostsByKeys(entries.map((entry)=>entry.postKey));return{period:normalizedPeriod,periods:contributingPeriods,entries,posts,meta:{aggregate:true,period:normalizedPeriod,periodCount:contributingPeriods.length,sourceEntryCount:sourceEntries.length,storedEntryCount:entries.length,totalExpectedPosts:entries.length,status:'complete',updatedAt:Math.max(0,...contributingPeriods.map((item)=>Number(item?.updatedAt)||0))}};},
  };

  const PopularSorter = {
    modes:new Set(['popular','published','title']),directions:new Set(['default','reverse']),
    normalize(mode='popular',direction='default'){return{mode:PopularSorter.modes.has(mode)?mode:'popular',direction:PopularSorter.directions.has(direction)?direction:'default'};},
    nextSelection(currentMode,currentDirection,selectedMode){const current=PopularSorter.normalize(currentMode,currentDirection),selected=PopularSorter.modes.has(selectedMode)?selectedMode:'popular';if(selected!==current.mode)return{mode:selected,direction:'default'};return{mode:current.mode,direction:current.direction==='default'?'reverse':'default'};},
    label(mode='popular',direction='default'){const value=PopularSorter.normalize(mode,direction),titles={popular:'Popular',published:'Publish date',title:'Post title'},reverse=value.direction==='reverse';return{title:titles[value.mode],arrow:reverse?'▲':'▼',spoken:reverse?(value.mode==='title'?'Z to A':'lowest first'):(value.mode==='title'?'A to Z':'highest first')};},
    sort(posts,entries,options={}){const value=PopularSorter.normalize(options.mode,options.direction);if(value.mode!=='popular')return PostSorter.sort(posts,{mode:value.mode,direction:value.direction});const reverse=value.direction==='reverse',byKey=entries instanceof Map?entries:new Map((entries||[]).map((entry)=>[entry.postKey,entry]));const sorted=[...posts].map((post,index)=>({post,index,entry:byKey.get(post.key)})).sort((a,b)=>{const af=Number(a.entry?.displayedFavoriteCount),bf=Number(b.entry?.displayedFavoriteCount),ak=Number.isFinite(af),bk=Number.isFinite(bf);if(ak!==bk)return ak?-1:1;if(ak&&af!==bf)return bf-af;const ar=Math.max(0,Number(a.entry?.rank)||0),br=Math.max(0,Number(b.entry?.rank)||0);if(ar&&br&&ar!==br)return ar-br;if(ar!==br)return ar?-1:1;return PostSorter.stableFallback(a,b);}).map(({post})=>post);return reverse?sorted.reverse():sorted;},
  };
  ...
  const PopularScanner = {
    ...
    resumeCoveredOffsets(existing={},action='scan'){const resuming=action==='resume'||action==='resume-update',retryMetadata=resuming&&Math.max(0,Number(existing.retryableCount)||0)>0;return new Set(resuming&&existing.workingRunId&&!retryMetadata?(existing.coveredOffsets||[]):[]);},
    ...
    async run(context,{signal,onProgress=()=>{},action='scan',jobId=''}={}){
      ...
      const existing=await Cache.getPopularPeriod(context.periodKey);const resumeAction=action==='resume'||action==='resume-update',updateAction=action==='update'||action==='resume-update',resume=resumeAction&&existing?.workingRunId;const runId=resume?String(existing.workingRunId):`popular-run-${Date.now()}-${Math.random().toString(36).slice(2)}`;const covered=PopularScanner.resumeCoveredOffsets(existing,action),checkpointContinuation=resume&&covered.size>0;let totalKnown=Boolean(context.totalKnown||Number(existing?.totalExpectedPosts)>0),total=Math.max(0,Number(context.totalPosts)||Number(existing?.totalExpectedPosts)||500),offsets=Array.from({length:Math.max(1,Math.ceil(total/Config.pageSize))},(_,index)=>index*Config.pageSize);let entriesWritten=checkpointContinuation?Math.max(0,Number(existing?.storedEntryCount)||0):0,detailFailed=checkpointContinuation?Math.max(0,Number(existing?.retryableCount)||0):0,pagesScanned=covered.size;const failedPostKeys=new Set(checkpointContinuation?(existing?.retryPostKeys||[]):[]);
      const base={periodKey:context.periodKey,domain:context.domain,period:context.period,date:context.date,label:context.label||PopularPeriod.label(context),status:updateAction?'updating':'building',workingRunId:runId,coveredOffsets:[...covered],totalExpectedPosts:total,storedEntryCount:entriesWritten,lastAttemptAt:Date.now(),lastError:'',updatedAt:Date.now()};await Cache.putPopularPeriod({...existing,...base});onProgress({page:Math.min(offsets.length,covered.size),totalPages:offsets.length,completed:entriesWritten,total,detailCompleted:0,detailTotal:0,message:resume?`Resuming Popular period · ${entriesWritten} / ${total} committed`:`Preparing Popular period · 0 / ${total}`});
      for(let pageIndex=0;pageIndex<offsets.length;pageIndex+=1){const offset=offsets[pageIndex];if(covered.has(offset))continue;if(signal.aborted)throw new DOMException('Aborted','AbortError');onProgress({page:pageIndex+1,totalPages:offsets.length,completed:entriesWritten,total,detailCompleted:0,detailTotal:0,message:`Reading Popular page ${pageIndex+1} / ${offsets.length}`});const html=await PawchiveAPI.fetchHtml(PopularPeriod.pageUrl(context,offset),signal,(message)=>onProgress({message}),{operation:'popular-scan',jobId,requestKind:'popular-page'});const doc=new DOMParser().parseFromString(html,'text/html');const parsed=PopularDOM.parseDocument(doc,context,offset);if(parsed.totalPosts){totalKnown=true;if(parsed.totalPosts!==total){total=parsed.totalPosts;offsets=Array.from({length:Math.max(1,Math.ceil(total/Config.pageSize))},(_,index)=>index*Config.pageSize);}}
  ...
  const PopularJobManager = {
    ...
    notifyProgress(){if(!PopularJobManager.progressPersistTimer){const persistDelay=document.visibilityState==='hidden'?1200:300;PopularJobManager.progressPersistTimer=setTimeout(()=>{PopularJobManager.progressPersistTimer=null;PopularJobManager.lastProgressPersistAt=Date.now();PopularJobManager.persist();},persistDelay);}if(PopularJobManager.progressNotifyTimer)return;const delay=document.visibilityState==='hidden'?750:80;PopularJobManager.progressNotifyTimer=setTimeout(()=>{PopularJobManager.progressNotifyTimer=null;PopularJobManager.notify();},delay);},
    flushProgress(){clearTimeout(PopularJobManager.progressNotifyTimer);clearTimeout(PopularJobManager.progressPersistTimer);PopularJobManager.progressNotifyTimer=null;PopularJobManager.progressPersistTimer=null;PopularJobManager.lastProgressPersistAt=Date.now();PopularJobManager.persist();PopularJobManager.notify();},
    ...
    descriptor(job,{resumeActive=false}={}){const status=resumeActive?'queued':job.status,requestedAction=resumeActive&&job.requestedAction!=='update'?'resume':job.requestedAction;return{periodKey:job.periodKey,context:{...(job.context||{})},requestedAction,status,resumeCheckpoint:Boolean(resumeActive||job.resumeCheckpoint),queuedAt:job.queuedAt,startedAt:job.startedAt,finishedAt:job.finishedAt,queueOrder:job.queueOrder,progress:{...(job.progress||{})},errorMessage:job.error?.message||job.errorMessage||''};},
    persistedPayload(){const descriptors=[...(PopularJobManager.activeJob?[PopularJobManager.descriptor(PopularJobManager.activeJob,{resumeActive:true})]:[]),...PopularJobManager.pendingJobs.map((job)=>PopularJobManager.descriptor(job)),...[...PopularJobManager.recentJobs.values()].map((job)=>PopularJobManager.descriptor(job))];return{version:3,descriptors,sequence:PopularJobManager.sequence,updatedAt:Date.now()};},
    persist(){try{const payload=PopularJobManager.persistedPayload(),serialized=JSON.stringify(payload);globalThis.sessionStorage?.setItem(Config.popularQueueSessionKey,serialized);GM_setValue(Config.popularQueueSessionKey,payload);}catch(error){Logger.info('Popular queue session could not be persisted.',error);}},
    restore(){if(PopularJobManager.restored)return;PopularJobManager.restored=true;PopularJobManager.paused=false;try{let data=null;const raw=globalThis.sessionStorage?.getItem(Config.popularQueueSessionKey);if(raw)try{data=JSON.parse(raw);}catch{}if(!data)data=GM_getValue(Config.popularQueueSessionKey,null);const descriptors=Array.isArray(data?.descriptors)?data.descriptors:[];PopularJobManager.sequence=Math.max(Number(data?.sequence)||0,PopularJobManager.sequence,...descriptors.map((item)=>Math.max(0,Number(item?.queueOrder)||0)),0);const seen=new Set();for(const descriptor of descriptors.sort((a,b)=>(Number(a?.queueOrder)||0)-(Number(b?.queueOrder)||0))){const periodKey=String(descriptor?.context?.periodKey||descriptor?.periodKey||'');if(!periodKey||seen.has(periodKey))continue;seen.add(periodKey);const resumable=Boolean(descriptor.resumeCheckpoint)||['running','interrupted'].includes(descriptor.status),recent=['complete','failed','stopped'].includes(descriptor.status)&&!resumable,job={id:`popular-job-${Date.now()}-${++PopularJobManager.sequence}`,periodKey,context:{...(descriptor.context||{}),periodKey},requestedAction:resumable&&descriptor.requestedAction!=='update'?'resume':descriptor.requestedAction||'scan',resumeCheckpoint:resumable,status:recent?descriptor.status:'queued',queuedAt:descriptor.queuedAt||Date.now(),startedAt:descriptor.startedAt||0,finishedAt:descriptor.finishedAt||0,queueOrder:descriptor.queueOrder||PopularJobManager.sequence,progress:{...(descriptor.progress||{}),message:recent?(descriptor.progress?.message||descriptor.status):resumable?(descriptor.progress?.message||'Restored interrupted period · resuming automatically'):descriptor.progress?.message||'Restored in Popular queue'},error:recent&&descriptor.errorMessage?new Error(descriptor.errorMessage):null,errorMessage:descriptor.errorMessage||'',controller:null,result:null};if(recent)PopularJobManager.recentJobs.set(periodKey,job);else PopularJobManager.pendingJobs.push(job);}PopularJobManager.pendingJobs.sort((a,b)=>a.queueOrder-b.queueOrder);PopularJobManager.persist();PopularJobManager.notify();PopularJobManager.pump();}catch(error){Logger.warn('Popular queue session could not be restored.',error);}},
    enqueue(context,requestedAction='scan'){if(!context?.periodKey)return{accepted:false,state:'invalid'};const existing=PopularJobManager.jobForPeriod(context.periodKey);if(existing&&['queued','running'].includes(existing.status))return{accepted:false,state:existing.status==='queued'?'already-queued':'already-active',job:existing,position:PopularJobManager.queuePosition(context.periodKey)};PopularJobManager.recentJobs.delete(context.periodKey);const job={id:`popular-job-${Date.now()}-${++PopularJobManager.sequence}`,periodKey:context.periodKey,context:{...context},requestedAction,resumeCheckpoint:false,status:'queued',queuedAt:Date.now(),queueOrder:PopularJobManager.sequence,progress:{message:'Waiting in Popular queue'},controller:null,result:null,error:null};PopularJobManager.pendingJobs.push(job);PopularJobManager.persist();PopularJobManager.notify();PopularJobManager.pump();return{accepted:true,state:PopularJobManager.activeJob===job?'started':'queued',job,position:PopularJobManager.queuePosition(context.periodKey)};},
    checkpointProgress(job,period={}){const total=Math.max(0,Number(job.context?.totalPosts)||Number(period?.totalExpectedPosts)||0),covered=[...new Set((period?.coveredOffsets||[]).map((value)=>Math.max(0,Number(value)||0)))],committed=Math.min(total||Number.MAX_SAFE_INTEGER,Math.max(0,Number(period?.storedEntryCount)||covered.length*Config.pageSize)),totalPages=Math.max(1,Math.ceil((total||committed||Config.pageSize)/Config.pageSize)),page=Math.min(totalPages,Math.max(covered.length,committed?Math.ceil(committed/Config.pageSize):0));return{total,completed:committed,totalPages,page,detailCompleted:0,detailTotal:0};},
    async start(job){job.status='running';job.startedAt=Date.now();job.controller=new AbortController();PopularJobManager.activeJob=job;PopularJobManager.persist();PopularJobManager.notify();let slot=false;try{await CatalogueJobManager.acquireMaintenanceSlot();slot=true;if(job.controller.signal.aborted)throw new DOMException('Aborted','AbortError');const checkpoint=await Cache.getPopularPeriod(job.periodKey);if(job.resumeCheckpoint){const checkpointProgress=PopularJobManager.checkpointProgress(job,checkpoint),prior=job.progress||{};job.progress={...checkpointProgress,...prior,total:Math.max(Number(checkpointProgress.total)||0,Number(prior.total)||0),completed:Math.max(Number(checkpointProgress.completed)||0,Number(prior.completed)||0),page:Math.max(Number(checkpointProgress.page)||0,Number(prior.page)||0),floorPercent:Math.max(0,Number(prior.floorPercent)||0)};PopularJobManager.flushProgress();}const scannerAction=job.resumeCheckpoint&&job.requestedAction==='update'?'resume-update':job.requestedAction;job.resumeCheckpoint=false;job.result=await PopularScanner.run(job.context,{signal:job.controller.signal,action:scannerAction,jobId:job.id,onProgress:(progress)=>{job.progress={...job.progress,...progress};PopularJobManager.notifyProgress();}});job.status='complete';job.progress={...job.progress,floorPercent:100,message:job.result.detailFailed?`Saved with ${job.result.detailFailed} metadata issue${job.result.detailFailed===1?'':'s'}`:'Popular period saved'};}
      catch(error){job.error=error;const paused=error.name==='AbortError'&&job.requeueAfterPause,interrupted=error.name==='AbortError'&&job.interruptedByNavigation;if(paused){job.status='queued';job.requestedAction=job.requestedAction==='update'?'update':'resume';job.progress={...job.progress,message:'Paused with the page · resuming automatically'};}else{job.status=interrupted?'interrupted':error.name==='AbortError'?'stopped':'failed';job.progress={...job.progress,message:interrupted?'Interrupted by navigation · Retry to continue':error.name==='AbortError'?'Popular scan stopped':error.message};}const period=await Cache.getPopularPeriod(job.periodKey);if(period&&!paused)await Cache.putPopularPeriod({...period,status:'partial',lastError:error.message,updatedAt:Date.now()});}
      finally{if(slot)CatalogueJobManager.releaseMaintenanceSlot();clearTimeout(PopularJobManager.progressNotifyTimer);PopularJobManager.progressNotifyTimer=null;job.finishedAt=Date.now();PopularJobManager.activeJob=null;if(job.requeueAfterPause&&!job.shutdown){job.requeueAfterPause=false;job.interruptedByNavigation=false;job.resumeCheckpoint=true;job.controller=null;PopularJobManager.pendingJobs.unshift(job);PopularJobManager.persist();PopularJobManager.notify();PopularJobManager.pump();}else if(!job.shutdown){PopularJobManager.recentJobs.set(job.periodKey,job);PopularJobManager.persist();PopularJobManager.notify();PopularJobManager.pump();}else PopularJobManager.notify();}return job;},
    ...
    clearCompleted(){for(const[key,job]of PopularJobManager.recentJobs)if(job.status==='complete')PopularJobManager.recentJobs.delete(key);PopularJobManager.persist();PopularJobManager.notify();},
    ...
    async cancelAllAndForget(){const active=PopularJobManager.activeJob;if(active){active.shutdown=true;active.controller?.abort();if(active.done)await Promise.allSettled([active.done]);}PopularJobManager.pendingJobs=[];PopularJobManager.activeJob=null;PopularJobManager.recentJobs.clear();try{globalThis.sessionStorage?.removeItem(Config.popularQueueSessionKey);}catch{}GM_deleteValue(Config.popularQueueSessionKey);PopularJobManager.notify();return true;},
    suspendForBfcache(){PopularJobManager.paused=true;const active=PopularJobManager.activeJob;if(active){active.requeueAfterPause=true;active.resumeCheckpoint=true;active.progress={...(active.progress||{}),floorPercent:Math.max(Number(active.progress?.floorPercent)||0,PopularQueuePanel.jobPercent(active))};}PopularJobManager.flushProgress();active?.controller?.abort();},
    resumeFromBfcache(){PopularJobManager.paused=false;PopularJobManager.notify();PopularJobManager.pump();},
    shutdown(){PopularJobManager.paused=true;clearTimeout(PopularJobManager.progressNotifyTimer);clearTimeout(PopularJobManager.progressPersistTimer);PopularJobManager.progressNotifyTimer=null;PopularJobManager.progressPersistTimer=null;const active=PopularJobManager.activeJob;if(active){active.resumeCheckpoint=true;active.progress={...(active.progress||{}),floorPercent:Math.max(Number(active.progress?.floorPercent)||0,PopularQueuePanel.jobPercent(active))};active.shutdown=true;}PopularJobManager.persist();active?.controller?.abort();},
  };
  ...
  const PopularQueuePanel = {
    jobPercent(job={}){const progress=job.progress||{},total=Math.max(0,Number(progress.total)||Number(job.context?.totalPosts)||Number(job.result?.period?.totalExpectedPosts)||0),completed=Math.max(0,Number(progress.completed)||0),detailTotal=Math.max(0,Number(progress.detailTotal)||0),detailCompleted=Math.min(detailTotal,Math.max(0,Number(progress.detailCompleted)||0)),calculated=total>0?Util.clamp(Math.round((completed+detailCompleted)/total*100),0,100):QueuePanelView.progress(job);return Math.max(calculated,Util.clamp(Math.round(Number(progress.floorPercent)||0),0,100));},
    aggregate(snapshot=PopularJobManager.snapshot()){const activeJobs=snapshot.active||[],active=activeJobs.length,waiting=(snapshot.pending||[]).length,recent=snapshot.recent||[],completed=recent.filter((job)=>job.status==='complete').length,failed=recent.filter((job)=>job.status==='failed').length,stopped=recent.filter((job)=>['stopped','interrupted'].includes(job.status)).length,finished=completed+failed+stopped,total=finished+active+waiting,progressUnits=finished+activeJobs.reduce((sum,job)=>sum+PopularQueuePanel.jobPercent(job)/100,0),percent=total?progressUnits/total*100:0;return{total,active,waiting,completed,failed,stopped,finished,remaining:active+waiting,percent};},
    row(job,kind){const progress=job.progress||{},periodKey=Util.escapeHtml(job.periodKey||''),message=progress.message||`${job.requestedAction||'scan'} · ${job.status||'queued'}`,actions=kind==='active'?'<button data-queue-action="stop">Stop</button>':kind==='waiting'?'<button data-queue-action="top">Move to top</button><button data-queue-action="remove">Remove</button>':kind==='complete'?'':'<button data-queue-action="retry">Retry</button><button data-queue-action="dismiss">Dismiss</button>';return`<div class="pmf-queue-row" data-period-key="${periodKey}"><div><strong>${Util.escapeHtml(PopularPeriod.label(job.context))}</strong><small>${Util.escapeHtml(message)}</small>${kind==='active'?`<progress max="100" value="${PopularQueuePanel.jobPercent(job)}" aria-label="${Util.escapeHtml(PopularPeriod.label(job.context))} progress"></progress>`:''}</div><span>${actions}</span></div>`;},
  ...
  const DataPortability = {
    ...
    auxiliarySettingKeys:[Config.postStatusFiltersKey,Config.creatorFilterStateKey,Config.creatorStatusFiltersKey,Config.creatorDirectoryModeKey,Config.popularModeKey,Config.popularAggregatePeriodKey],
...
sync() {
    ...
    OverlayManager.stack.forEach(
        (entry, index) => {
            ...
            if(entry.root?.style)entry.root.style.setProperty('z-index',String(4000+index*20),'important');
            entry.root?.setAttribute?.('aria-hidden',String(index!==OverlayManager.stack.length-1));
        }
    );
},
    install(signal) {
  ...
  const CardRenderer = {
    ...
    clone(post) {
      ...
      if (post.thumbnailUrl) {
        imageContainer?.querySelectorAll?.('source')?.forEach?.((source)=>source.remove());
        if (!image && imageContainer) {
          image = document.createElement('img'); image.className = 'post-card__image'; imageContainer.append(image);
        }
        if (image) {for(const attribute of ['srcset','data-src','data-srcset','data-lazy-src','data-original'])image.removeAttribute?.(attribute);image.removeAttribute?.('src');image.src=post.thumbnailUrl;image.alt='';image.loading='eager';image.decoding='async';imageContainer?.classList.remove('pmf-fallback-thumb');}
      } else if (image) {
        image.remove();
    ...
    fallback(post) {
      ...
      card.dataset.id = post.id;
      card.dataset.pmfPostKey = String(post.key || '');
      card.innerHTML = `<a class="image-link" href="${Util.escapeHtml(post.postUrl)}"><header class="post-card__header">${Util.escapeHtml(post.title)}</header><div class="post-card__image-container ${post.thumbnailUrl ? '' : 'pmf-fallback-thumb'}">${post.thumbnailUrl ? `<img class="post-card__image" loading="eager" decoding="async" alt="" src="${Util.escapeHtml(post.thumbnailUrl)}">` : ''}</div><footer class="post-card__footer"><div><time datetime="${Util.escapeHtml(post.publishedAt)}">${Util.escapeHtml(Util.formatDate(post.publishedAt))}</time><div>${post.attachmentCount ? `${post.attachmentCount} attachment${post.attachmentCount === 1 ? '' : 's'}` : 'No attachments'}</div></div></footer></a>`;
      CardRenderer.normalizeThumbnailGeometry(card);
      BadgeRenderer.apply(card, post);
      StatusBadgeRenderer.apply(card,post);
      SeenCardTreatment.apply(card,post);
  ...
  const Paginator = {
    ...
    render(count) {
      ...
      const renderInto=(countNode,controls,host)=>{
        ...
        const add=(label,action,page,disabled,aria)=>{const button=document.createElement('button');button.type='button';button.textContent=label;button.disabled=disabled;button.setAttribute('aria-label',aria);button.dataset.pmfPageAction=action;if(action==='page')button.dataset.pmfPage=String(page);else button.classList.add('pmf-page-nav');controls.append(button);};
  ...
  const MissingAttachmentMaintenance = {
    ...
    openScopeDialog(opener) {
      const backdrop = SettingsUI.el('div', 'pmf-modal-backdrop pmf-maintenance-scope-backdrop');backdrop.dataset.pmfOwned='true';
      const dialog = SettingsUI.el('section', 'pmf-dialog pmf-small-dialog pmf-surface pmf-maintenance-scope-dialog');
      ...
      GlobalUI.ensureHost().append(backdrop);UI.own(backdrop);
      const overlay = OverlayManager.open({ id:`pmf-maintenance-scope-${Date.now()}`, owner:'maintenance-scope', node: dialog, root: backdrop, opener, modal: true });
      ...
      backdrop.addEventListener('click', async (event) => {
        const action = event.target.closest('[data-maintenance-scope-action]')?.dataset.maintenanceScopeAction;if(action)event.stopPropagation();
  ...
  const MaintenanceCommandRunner = {
    modes:new Set(['watch-all','watch-missing','resume-missing','retry-missing','start-missing','watch-profiles','resume-profiles','retry-profiles','start-profiles']),
    mode:'',interval:null,running:null,lastStartedAt:0,lastInventoryCheckAt:0,lastProfileCheckAt:0,inventoryCheckIntervalMs:5*60*1000,statusInterval:null,statusUnsubscribers:[],originalTitle:'',statusNode:null,lastStatusKey:'',
    formatEta(ms=0){const seconds=Math.max(0,Math.round(Number(ms)||0)/1000);if(!seconds)return'calculating';const hours=Math.floor(seconds/3600),minutes=Math.floor((seconds%3600)/60),rest=Math.floor(seconds%60);return hours?`${hours}h ${minutes}m`:minutes?`${minutes}m ${rest}s`:`${rest}s`;},
    statusSnapshot(){const missing=MissingAttachmentMaintenance.snapshot(),profiles=CreatorProfileRepairManager.snapshot(),missingState=()=>{const total=Math.max(Number(missing.total)||0,Number(missing.completed)||0),completed=Math.max(0,Number(missing.completed)||0),remaining=Math.max(0,Number(missing.remaining)||0),percent=total?Util.clamp(Math.round(completed/total*100),0,100):0;return{kind:'Missing attachments',running:missing.running,total,completed,remaining,failed:(Number(missing.failed)||0)+(Number(missing.permanentFailed)||0),percent,etaMs:Number(missing.etaMs)||0,message:missing.message||'',planning:Boolean(missing.planning),heartbeatAt:Date.now()};},profileState=()=>{const total=Math.max(Number(profiles.total)||0,Number(profiles.completed)||0),completed=Math.max(0,Number(profiles.completed)||0),remaining=Math.max(0,Number(profiles.remaining)||0),percent=total?Util.clamp(Math.round(completed/total*100),0,100):0,rate=Math.max(0,Number(profiles.rate)||0);return{kind:'Creator profiles',running:profiles.running,total,completed,remaining,failed:Number(profiles.failed)||0,percent,etaMs:rate&&remaining?Math.round(remaining/rate*1000):0,message:profiles.message||'',planning:false,heartbeatAt:Date.now()};};if(missing.running)return missingState();if(profiles.running)return profileState();if(missing.remaining||missing.failed||missing.stopped)return missingState();if(profiles.remaining||profiles.failed||profiles.stopped)return profileState();const last=profiles.total?`Creator-profile pass complete · ${profiles.completed}/${profiles.total}`:missing.total?`Missing-attachment pass complete · ${missing.completed}/${missing.total}`:'Checking maintenance checkpoints and stored metadata…';return{kind:'Waiting',running:false,total:0,completed:0,remaining:0,failed:0,percent:0,etaMs:0,message:last,planning:false,heartbeatAt:Date.now()};},
    statusText(state=MaintenanceCommandRunner.statusSnapshot()){const progress=state.total?`${state.completed} / ${state.total} (${state.percent}%)`:(state.planning?'Planning stored posts':'Idle'),remaining=state.total?` | ${state.remaining} left`:'',failed=state.failed?` | ${state.failed} failed`:'',eta=state.remaining?` | ETA ${MaintenanceCommandRunner.formatEta(state.etaMs)}`:'';return`${state.kind} | ${progress}${remaining}${failed}${eta}${state.message?` | ${state.message}`:''}`;},
    publishStatus(){if(!MaintenanceCommandRunner.mode)return null;const state=MaintenanceCommandRunner.statusSnapshot(),text=MaintenanceCommandRunner.statusText(state),title=`PMF Runner | ${text}`;document.title=title;document.documentElement?.setAttribute?.('data-pmf-maintenance-status',JSON.stringify({...state,text,version:Config.version,mode:MaintenanceCommandRunner.mode}));if(MaintenanceCommandRunner.statusNode?.isConnected){MaintenanceCommandRunner.statusNode.textContent=text;MaintenanceCommandRunner.statusNode.dataset.running=String(Boolean(state.running));}const key=`${state.kind}|${state.completed}|${state.total}|${state.remaining}|${state.failed}|${state.running}|${state.message}`;if(key!==MaintenanceCommandRunner.lastStatusKey){MaintenanceCommandRunner.lastStatusKey=key;Logger.info({operation:'metadata-maintenance-progress',mode:MaintenanceCommandRunner.mode,...state});}return state;},
    installStatusBridge(){if(!MaintenanceCommandRunner.originalTitle)MaintenanceCommandRunner.originalTitle=document.title;MaintenanceCommandRunner.statusNode=document.querySelector('[data-pmf-maintenance-banner]');if(!MaintenanceCommandRunner.statusNode){const node=document.createElement('div');node.dataset.pmfMaintenanceBanner='true';node.dataset.pmfOwned='true';node.setAttribute('role','status');node.setAttribute('aria-live','polite');node.style.cssText='position:fixed;right:10px;bottom:10px;z-index:2147483600;max-width:min(620px,calc(100vw - 20px));padding:8px 10px;border:1px solid #596168;border-radius:5px;background:#14181bea;color:#f1f2f3;font:12px/1.35 Arial,sans-serif;box-shadow:0 8px 28px #0009;pointer-events:none';GlobalUI.ensureHost().append(node);MaintenanceCommandRunner.statusNode=node;}MaintenanceCommandRunner.statusUnsubscribers.forEach((unsubscribe)=>unsubscribe?.());MaintenanceCommandRunner.statusUnsubscribers=[MissingAttachmentMaintenance.subscribe(()=>MaintenanceCommandRunner.publishStatus()),CreatorProfileRepairManager.subscribe(()=>MaintenanceCommandRunner.publishStatus())];clearInterval(MaintenanceCommandRunner.statusInterval);MaintenanceCommandRunner.statusInterval=setInterval(()=>MaintenanceCommandRunner.publishStatus(),1000);MaintenanceCommandRunner.publishStatus();},
    uninstallStatusBridge(){clearInterval(MaintenanceCommandRunner.statusInterval);MaintenanceCommandRunner.statusInterval=null;MaintenanceCommandRunner.statusUnsubscribers.splice(0).forEach((unsubscribe)=>unsubscribe?.());MaintenanceCommandRunner.statusNode?.remove?.();MaintenanceCommandRunner.statusNode=null;document.documentElement?.removeAttribute?.('data-pmf-maintenance-status');if(MaintenanceCommandRunner.originalTitle)document.title=MaintenanceCommandRunner.originalTitle;MaintenanceCommandRunner.originalTitle='';MaintenanceCommandRunner.lastStatusKey='';},
    ...
    profileState(){return{checkpoint:CreatorProfileRepairManager.checkpoint(),snapshot:CreatorProfileRepairManager.snapshot()};},
    async chooseMissing(mode){const{checkpoint,snapshot}=MaintenanceCommandRunner.checkpointState();if(mode==='start-missing')return{task:MissingAttachmentMaintenance.run({scope:'all'}),snapshot};if(mode==='retry-missing')return{task:MissingAttachmentMaintenance.retryFailed(),snapshot};if(mode==='resume-missing')return{task:MissingAttachmentMaintenance.resume(),snapshot};if(mode!=='watch-missing')return{task:null,snapshot};if(!checkpoint)return{task:MissingAttachmentMaintenance.run({scope:'all'}),snapshot};if(snapshot.failed>0&&!snapshot.remaining)return{task:MissingAttachmentMaintenance.retryFailed(),snapshot};if(snapshot.remaining>0||snapshot.stopped)return{task:MissingAttachmentMaintenance.resume(),snapshot};const now=Date.now();if(now-MaintenanceCommandRunner.lastInventoryCheckAt<MaintenanceCommandRunner.inventoryCheckIntervalMs)return{task:null,snapshot};MaintenanceCommandRunner.lastInventoryCheckAt=now;const inventory=await Cache.countMissingAttachmentStats();return inventory.unknown>0?{task:MissingAttachmentMaintenance.run({scope:'all',estimatedStoredPosts:inventory.total}),snapshot}:{task:null,snapshot};},
    async chooseProfiles(mode){const{checkpoint,snapshot}=MaintenanceCommandRunner.profileState();if(mode==='start-profiles')return{task:CreatorProfileRepairManager.run(),snapshot};if(mode==='retry-profiles')return{task:CreatorProfileRepairManager.retryFailed(),snapshot};if(mode==='resume-profiles')return{task:CreatorProfileRepairManager.resume(),snapshot};if(mode!=='watch-profiles')return{task:null,snapshot};if(!checkpoint){MaintenanceCommandRunner.lastProfileCheckAt=Date.now();return{task:CreatorProfileRepairManager.run(),snapshot};}if(snapshot.failed>0&&!snapshot.remaining)return{task:CreatorProfileRepairManager.retryFailed(),snapshot};if(snapshot.remaining>0||snapshot.stopped)return{task:CreatorProfileRepairManager.resume(),snapshot};const now=Date.now();if(now-MaintenanceCommandRunner.lastProfileCheckAt<MaintenanceCommandRunner.inventoryCheckIntervalMs)return{task:null,snapshot};MaintenanceCommandRunner.lastProfileCheckAt=now;return{task:CreatorProfileRepairManager.run(),snapshot};},
    async chooseTask(mode){if(mode==='watch-all'){const missing=await MaintenanceCommandRunner.chooseMissing('watch-missing');if(missing.task)return missing;return MaintenanceCommandRunner.chooseProfiles('watch-profiles');}if(mode.endsWith('-profiles'))return MaintenanceCommandRunner.chooseProfiles(mode);return MaintenanceCommandRunner.chooseMissing(mode);},
    async execute(mode=MaintenanceCommandRunner.mode){
      if(MissingAttachmentMaintenance.active)return MissingAttachmentMaintenance.active.promise;if(CreatorProfileRepairManager.active)return CreatorProfileRepairManager.active.promise;if(MaintenanceCommandRunner.running)return MaintenanceCommandRunner.running;
      const operation=(async()=>{MaintenanceCommandRunner.publishStatus();const{task,snapshot}=await MaintenanceCommandRunner.chooseTask(mode);if(!task){MaintenanceCommandRunner.publishStatus();return snapshot;}MaintenanceCommandRunner.lastStartedAt=Date.now();MaintenanceCommandRunner.publishStatus();return await task;})().catch((error)=>{Logger.error('External metadata maintenance command failed.',error);return mode.endsWith('-profiles')?CreatorProfileRepairManager.snapshot():MissingAttachmentMaintenance.snapshot();}).finally(()=>{if(MaintenanceCommandRunner.running===operation)MaintenanceCommandRunner.running=null;MaintenanceCommandRunner.publishStatus();});
      MaintenanceCommandRunner.running=operation;return operation;
    },
    start(url=location.href){
      const mode=MaintenanceCommandRunner.parse(url);if(!mode)return false;MaintenanceCommandRunner.mode=mode;MaintenanceCommandRunner.lastInventoryCheckAt=0;MaintenanceCommandRunner.lastProfileCheckAt=0;document.documentElement?.setAttribute?.('data-pmf-maintenance-mode',mode);Logger.info({operation:'metadata-maintenance-command',mode});MaintenanceCommandRunner.installStatusBridge();
      queueMicrotask(()=>MaintenanceCommandRunner.execute(mode));if(['watch-all','watch-missing','watch-profiles'].includes(mode)){clearInterval(MaintenanceCommandRunner.interval);MaintenanceCommandRunner.interval=setInterval(()=>MaintenanceCommandRunner.execute(mode),20000);MaintenanceCommandRunner.interval?.unref?.();}
      return true;
    },
    stop(){clearInterval(MaintenanceCommandRunner.interval);MaintenanceCommandRunner.interval=null;MaintenanceCommandRunner.uninstallStatusBridge();MaintenanceCommandRunner.mode='';document.documentElement?.removeAttribute?.('data-pmf-maintenance-mode');},
  };
  ...
  const UI = {
    ...
    toggleSortMenu(force,initialFocus='') {
      ...
      const sortModes=App.pageKind==='popular'?[['popular','Popular'],['published','Publish date'],['title','Post title']]:[['published','Publish date'],['title','Post title']];sortModes.forEach(([mode,label])=>{
...
UI.closeSettings('reopen');const checked=(value)=>value?'checked':'';const selected=(value,current)=>value===current?'selected':'';const backdrop=document.createElement('div');backdrop.className='pmf-modal-backdrop';App.ui.settingsBadgeTypes=Util.clone(Settings.value.catalogueBadges.types);App.ui.settingsCreatorBadgeTypes=Util.clone(Settings.value.creatorCardBadges.types);App.ui.settingsStatusBadgeTypes=Util.clone(Settings.value.postStatusBadges.types);App.ui.settingsPreview={displayMode:App.displayMode,compactCardScale:Settings.value.compactCardScale,compactThumbnailAspectRatio:Settings.value.compactThumbnailAspectRatio,postAttachmentBadgeSize:Settings.value.postAttachmentBadgeSize,filteredPage:App.filteredPage,postStatusBadgeSize:Settings.value.postStatusBadgeSize,seenCardTreatment:Util.clone(Settings.value.seenCardTreatment),committed:false};
  ...
  const SettingsUI = {
    ...
    buildScanning(draft) {
      const panel = SettingsUI.panel('scanning', 'Scanning & detection');
      const behavior = SettingsUI.section('Scan behavior');
      behavior.append(SettingsUI.toggle('synchronizeNativeFavorites', draft.synchronizeNativeFavorites, 'Synchronize native favorites during Scan and Update'));
      const detection = SettingsUI.section('Creator catalogue detection');
      detection.append(
      ...
      panel.append(behavior, detection, queue);
      return panel;
    },
    buildData(draft) {
      ...
      const maintenance=SettingsUI.section('Catalogue maintenance');
      const maintenanceHelp=SettingsUI.el('p','pmf-help','Open a categorized maintenance workspace for missing-attachment checks, creator-profile repair, native-favorite synchronization, and catalogue reset actions. Each operation explains what it changes.');
      const maintenanceActions=SettingsUI.el('div','pmf-settings-actions');
      maintenanceActions.append(SettingsUI.action('Open catalogue maintenance','open-catalogue-maintenance'));
      maintenance.append(maintenanceHelp,maintenanceActions);
      const portability=SettingsUI.section('Backup and reset');
      const portabilityHelp=SettingsUI.el('p','pmf-help','Export creator catalogues, Popular Posts snapshots, statuses, saved UI state, settings, and both post and creator presets. Import can merge catalogue data or replace the current local catalogue.');
      ...
      panel.append(requests,maintenance,portability);
      return panel;
    },
    showChild(name) {
    ...
    open() {
      ...
      backdrop.addEventListener('click', async (event) => {
        ...
        if (action === 'open-catalogue-maintenance') CatalogueMaintenanceUI.open(event.target);
        if (action === 'reset') {
  ...
  const CatalogueMaintenanceUI = {
    open(opener){
      const backdrop=SettingsUI.el('div','pmf-modal-backdrop');
      const dialog=SettingsUI.el('section','pmf-dialog pmf-maintenance-center pmf-surface');
      dialog.setAttribute('role','dialog');dialog.setAttribute('aria-modal','true');dialog.setAttribute('aria-label','Catalogue maintenance');
      const currentCreator=Boolean(App.context?.creatorKey);
      dialog.innerHTML=`<header><strong>Catalogue maintenance</strong><button type="button" class="pmf-icon-close" data-maintenance-center-action="close" aria-label="Close">×</button></header><div class="pmf-editor-body pmf-maintenance-center-body"><section class="pmf-settings-section"><h3>Missing-attachment metadata</h3><p class="pmf-help" data-missing-attachment-inventory>Missing-attachment metadata inventory: counting stored posts…</p><p class="pmf-help">Checks posts whose missing-attachment state is still unknown. Resume continues a stopped operation; Retry failed only processes retryable failures.</p><div class="pmf-settings-actions"><button type="button" data-settings-action="update-missing-attachment-metadata">Start or choose scope</button><button type="button" data-settings-action="stop-missing-attachment-metadata">Stop</button><button type="button" data-settings-action="resume-missing-attachment-metadata">Resume</button><button type="button" data-settings-action="retry-failed-missing-attachment-metadata">Retry failed</button></div></section><section class="pmf-settings-section"><h3>Creator profile metadata</h3><p class="pmf-help">Repairs weak creator names, profile URLs, avatars, banners, service labels, and public favorite counts without deleting stored posts.</p><div class="pmf-settings-actions"><button type="button" data-settings-action="repair-creator-profile-metadata">Repair creator profiles</button><button type="button" data-settings-action="stop-creator-profile-repair">Stop</button><button type="button" data-settings-action="resume-creator-profile-repair">Resume</button><button type="button" data-settings-action="retry-failed-creator-profile-repair">Retry failed</button></div></section><section class="pmf-settings-section"><h3>Native favorites</h3><p class="pmf-help">Refreshes Pawchive's native favorite membership and merges it with local Favorite status.</p><div class="pmf-settings-actions"><button type="button" data-settings-action="sync-favorites">Synchronize now</button><button type="button" data-settings-action="stop-favorites">Stop synchronization</button></div><p class="pmf-help" data-favorite-sync-center-status></p></section><section class="pmf-settings-section pmf-maintenance-danger"><h3>Destructive actions</h3><p class="pmf-help">These remove locally stored scan data. They do not delete anything from Pawchive.</p><div class="pmf-settings-actions">${currentCreator?'<button type="button" class="pmf-danger" data-settings-action="clear-creator">Clear this creator scan</button>':''}<button type="button" class="pmf-danger" data-settings-action="clear-all">Clear all full catalogue scans</button></div></section><p class="pmf-maintenance-status" data-maintenance-status role="status"></p></div><footer><span></span><button type="button" data-maintenance-center-action="close">Close</button></footer>`;
      backdrop.append(dialog);GlobalUI.ensureHost().append(backdrop);UI.own(backdrop);
      let maintenanceUnsubscribe=()=>{},favoriteUnsubscribe=()=>{};
      const overlay=OverlayManager.open({node:dialog,root:backdrop,opener,modal:true,onClose:()=>{maintenanceUnsubscribe();favoriteUnsubscribe();}});
      maintenanceUnsubscribe=SettingsUI.bindMaintenance(backdrop);
      favoriteUnsubscribe=FavoriteSyncCoordinator.subscribe((state)=>{const status=dialog.querySelector('[data-favorite-sync-center-status]'),start=dialog.querySelector('[data-settings-action="sync-favorites"]'),stop=dialog.querySelector('[data-settings-action="stop-favorites"]');if(start)start.hidden=state.running;if(stop)stop.hidden=!state.running;if(status)status.textContent=state.running?(state.message||'Synchronizing native favorites…'):state.lastAttemptStatus==='failed'?`Last synchronization failed: ${state.lastError||'Unknown error'}`:state.completedAt?`Last synchronized ${new Date(state.completedAt).toLocaleString()} · ${state.favoriteCount||0} favorites`:state.lastAttemptStatus==='stopped'?'Synchronization stopped.':'No synchronization has run yet.';});
      backdrop.addEventListener('click',async(event)=>{const close=event.target.closest('[data-maintenance-center-action]')?.dataset.maintenanceCenterAction,action=event.target.closest('[data-settings-action]')?.dataset.settingsAction;if(close==='close'){OverlayManager.close(overlay,'close');return;}if(!action)return;if(action==='sync-favorites')await FavoriteSyncCoordinator.run({manual:true});if(action==='stop-favorites')FavoriteSyncCoordinator.stop();if(action==='update-missing-attachment-metadata')MissingAttachmentMaintenance.openScopeDialog(event.target);if(action==='stop-missing-attachment-metadata')MissingAttachmentMaintenance.stop();if(action==='resume-missing-attachment-metadata')SettingsUI.runMaintenance(MissingAttachmentMaintenance.resume());if(action==='retry-failed-missing-attachment-metadata')SettingsUI.runMaintenance(MissingAttachmentMaintenance.retryFailed());if(action==='repair-creator-profile-metadata')SettingsUI.runMaintenance(CreatorProfileRepairManager.run());if(action==='stop-creator-profile-repair')CreatorProfileRepairManager.stop();if(action==='resume-creator-profile-repair')SettingsUI.runMaintenance(CreatorProfileRepairManager.resume());if(action==='retry-failed-creator-profile-repair')SettingsUI.runMaintenance(CreatorProfileRepairManager.retryFailed());if(action==='clear-creator')UI.confirmClear('catalogue-creator');if(action==='clear-all')UI.confirmClear('catalogue-all');});
      return overlay;
    },
  };
    ...
    handle(event) {
      ...
      if (status) {
        if (status === 'native-scanned') CreatorIndexUI.nativeScannedFilter = CreatorIndexUI.nativeScannedFilter === 'no-match' ? 'off' : 'no-match';
          ...
          CreatorIndexUI.statusFilters[status] = CreatorStatusFilters.cycle(CreatorIndexUI.statusFilters[status],status);
  ...
  const CreatorSettingsUI = {
    ...
    open(opener) {
      const draft=Util.clone(Settings.value);const backdrop=SettingsUI.el('div','pmf-modal-backdrop');const dialog=SettingsUI.el('section','pmf-dialog pmf-settings-dialog pmf-surface');dialog.setAttribute('role','dialog');dialog.setAttribute('aria-modal','true');dialog.setAttribute('aria-label','Media Filter settings');const header=SettingsUI.el('header');header.append(SettingsUI.el('strong','','Media Filter settings'));const close=SettingsUI.action('×','cancel');close.className='pmf-icon-close';header.append(close);const layout=SettingsUI.el('div','pmf-settings-layout');const nav=SettingsUI.el('nav');const content=SettingsUI.el('div','pmf-settings-content');const panels=[SettingsUI.buildGeneral(draft),SettingsUI.buildDetection(draft),SettingsUI.buildScanning(draft),SettingsUI.buildData(draft)];[['general','General'],['default-detection','Default detection'],['scanning','Scanning & detection'],['data','Data & performance']].forEach(([key,label])=>{const button=SettingsUI.action(label,'tab');button.dataset.creatorSettingsTab=key;nav.append(button);});content.append(...panels);layout.append(nav,content);const footer=SettingsUI.el('footer');footer.append(SettingsUI.el('span'),SettingsUI.action('Cancel','cancel'));const save=SettingsUI.action('Save & apply','save');save.className='pmf-primary';footer.append(save);dialog.append(header,layout,footer);backdrop.append(dialog);CreatorIndexUI.root.append(backdrop);
  ...
  const CreatorFilterUI = {
    ...
    open(opener) {
      ...
      popover.innerHTML=`<div class="pmf-popover-title">Creator filters</div><label class="pmf-match-mode pmf-service-filter"><select data-creator-filter-service aria-label="Creator service"><option value="all">All services</option><option value="patreon">Service: Patreon</option><option value="fanbox">Service: Pixiv Fanbox</option></select></label><label class="pmf-match-mode"><span>Match selected filters</span><select data-creator-filter-match aria-label="Match selected creator filters"><option value="all">All</option><option value="any">Any</option></select></label><button type="button" class="pmf-preset-selector" data-creator-filter-presets aria-haspopup="dialog"><span>Preset: ${Util.escapeHtml(activePreset?.name||'Default')}</span><b>›</b></button><div class="pmf-popover-section">General</div>${row('videos','Videos')}${row('images','Images')}${row('archives','Archives')}${row('projectFiles','Project files')}${row('externalLinks','External links')}<div class="pmf-popover-section">Advanced</div>${row('customExtensions','Custom extensions')}<div class="pmf-filter-row"><label><input type="checkbox" data-creator-filter-toggle="customRules" ${rulesEnabled?'checked':''}><span>Advanced rules</span></label><button type="button" class="pmf-row-chevron" data-creator-filter-editor="customRules" aria-label="Configure advanced rules">›</button></div><div class="pmf-filter-row"><label><input type="checkbox" data-creator-filter-toggle="publishedDate" ${date.enabled?'checked':''}><span>Date published</span></label><button type="button" class="pmf-row-chevron" data-creator-filter-editor="publishedDate" aria-label="Configure date published">›</button></div><label class="pmf-creator-partial-option"><input type="checkbox" data-creator-filter-partial ${state.includePartialLowerBounds?'checked':''}><span>Include safe partial lower bounds</span></label><p class="pmf-help">Percentages, ≤, and Between require complete Local catalogues. Service is always combined with the selected filters.</p>`;
      ...
      popover.addEventListener('change',(event)=>{
        ...
        else if(toggle==='customRules')next.customRulesEnabled=event.target.checked;
        else if(toggle==='customExtensions')next.media.customExtensions.enabled=event.target.checked;
    ...
    openAdvancedRules(opener) {
      ...
      shell.backdrop.addEventListener('click',(event)=>{const action=event.target.closest('[data-creator-dialog-action]')?.dataset.creatorDialogAction;if(action==='cancel'||action==='discard')OverlayManager.close(shell.overlay,action);if(action==='apply'){collect();const invalid=draft.customRules.find((rule)=>!CreatorCustomRule.valid(rule));if(invalid){error.textContent='Complete every advanced rule.';return;}draft.customRules=draft.customRules.map((rule,index)=>({...rule,enabled:true,join:index===0?'if':rule.join}));CreatorFilterUI.commit(draft);OverlayManager.close(shell.overlay,'apply');}});
    },
    ...
    openPresets(opener) {
      ...
      render();
      shell.body.addEventListener('click',(event)=>{const activate=event.target.closest('[data-creator-preset-activate]');if(activate){applyPreset(activate.dataset.creatorPresetActivate);OverlayManager.close(shell.overlay,'apply');return;}const menu=event.target.closest('[data-creator-preset-menu]');if(!menu)return;const id=menu.dataset.creatorPresetMenu;const items=[{value:'rename',label:'Rename'},{value:'duplicate',label:'Duplicate'},{value:'delete',label:'Delete'}];AnchoredMenu.open(menu,items,{owner:'artists:creator-preset-actions',onSelect:(action)=>{showError('');if(action==='rename'){const preset=record.presets.find((item)=>item.id===id);queueMicrotask(()=>CreatorFilterUI.openPresetName('Rename creator preset',preset?.name||'',menu,(name)=>{const result=CreatorPresets.rename(record,id,name);if(result.valid){record=result.record;render();}return result;}));return;}if(action==='duplicate'){const result=CreatorPresets.duplicate(record,id);record=result.record;if(result.valid){CreatorFilterUI.commit(result.preset.state);render();}else showError(result.message);return;}if(action==='delete'){const wasActive=record.activeId===id;const result=CreatorPresets.remove(record,id);record=result.record;if(!result.valid){render();showError(result.message);return;}if(wasActive)applyPreset('default');render();}}});});
  ...
  const AnchoredMenu = {
    open(opener,items,{selected='',onSelect=()=>{},anchor=opener,owner=`anchored:${opener?.dataset?.creatorIndexAction||opener?.id||opener?.getAttribute?.('aria-label')||'menu'}`}={}){
      ...
      const mobile=Boolean(globalThis.matchMedia?.('(max-width: 760px)')?.matches);
      const preferredWidth=mobile?Math.max(220,rect.width,Math.min(286,viewportWidth-16)):Math.max(190,rect.width);
  ...
  const CreatorSortUI = {
    ...
    apply(next) {
      CreatorIndexUI.sort=CreatorSorter.normalize(next);if(CreatorIndexUI.sort.mode==='advanced')CreatorSortUI.rememberAdvanced(CreatorIndexUI.sort.advancedType,CreatorIndexUI.sort.advancedMethod);const button=CreatorIndexUI.toolbar?.querySelector?.('[data-creator-index-action="sort"]');if(button){const label=button.querySelector('.pmf-sort-label'),arrow=button.querySelector('.pmf-sort-direction');if(label)label.textContent=`Sort: ${CreatorSortUI.label().replace(/\s[▲▼]$/,'')}`;if(arrow)arrow.textContent=CreatorIndexUI.sort.direction==='asc'?'▲':'▼';}CreatorIndexUI.page=1;CreatorIndexUI.invalidateFilteredCache('sort');CreatorIndexUI.render();CreatorIndexUI.retainSession();
    },
    open(opener) {
  ...
  const PopularPageController = {
    root:null,found:null,page:null,context:null,controller:null,unsubscribe:null,observer:null,nativeSnapshot:null,lastCompletedAt:0,queueOpen:Boolean(GM_getValue(Config.popularQueueOpenKey,false)),refreshRevision:0,observerSchedule:null,layoutSchedule:null,rebinding:false,nativePagePending:null,nativePageTimer:null,dataLoading:false,aggregateLinkStates:new Map(),
    mounted(){return Boolean(PopularPageController.root?.isConnected&&PopularPageController.context?.periodKey);},
    setQueueOpen(value){PopularPageController.queueOpen=Boolean(value);GM_setValue(Config.popularQueueOpenKey,PopularPageController.queueOpen);PopularQueuePanel.render();return PopularPageController.queueOpen;},
    ...
    saveNative(found){return{grid:PopularPageController.nativeState(found.grid),paginators:(found.paginators||[]).map(PopularPageController.nativeState),counts:(found.countNodes||[]).map(PopularPageController.nativeState),navs:(found.navContainers||[]).filter((node)=>PopularDOM.isSafeNativeControlGroup(node,found.grid)).map(PopularPageController.nativeState),notices:(found.restrictionNotices||[]).map(PopularPageController.nativeState),heading:found.heading?{node:found.heading,text:String(found.heading.textContent||'')}:null};},
    ...
    restoreNative(){PopularPageController.restoreAggregatePeriodControls();PopularPageController.restoreNode(PopularPageController.nativeSnapshot?.grid);for(const item of PopularPageController.nativeSnapshot?.paginators||[])PopularPageController.restoreNode(item);for(const item of PopularPageController.nativeSnapshot?.counts||[])PopularPageController.restoreNode(item);for(const item of PopularPageController.nativeSnapshot?.navs||[])PopularPageController.restoreNode(item);for(const item of PopularPageController.nativeSnapshot?.notices||[])PopularPageController.restoreNode(item);const heading=PopularPageController.nativeSnapshot?.heading;if(heading?.node?.isConnected)heading.node.textContent=heading.text;},
    ...
    async load(context,found,{signal}={}){
      ...
      App.popularMeta=await Cache.getPopularPeriod(context.periodKey);PopularPageController.assertMountCurrent(context,signal);let entries=await Cache.getPopularEntries(context.periodKey);PopularPageController.assertMountCurrent(context,signal);App.popularEntries=new Map(entries.map((entry)=>[String(entry.postKey),entry]));let posts=await Cache.getPostsByKeys(entries.map((entry)=>entry.postKey));PopularPageController.assertMountCurrent(context,signal);App.catalog=new Map([...posts.values()].map((post)=>[String(post.key),post]));await PopularPageController.loadStatuses(context,entries);PopularPageController.assertMountCurrent(context,signal);
      ...
      App.filteredPage=Math.max(1,Util.parseInteger(saved?.filteredPage,1));App.filteredFirstResultIndex=Math.max(0,Util.parseInteger(saved?.filteredFirstResultIndex,0));App.filteredAnchorId=String(saved?.filteredAnchorId||'');const popularSort=PopularSorter.normalize(saved?.sortMode,saved?.sortDirection);App.sortMode=popularSort.mode;App.sortDirection=popularSort.direction;App.totalPosts=Math.max(Number(App.popularMeta?.totalExpectedPosts)||0,entries.length,found.totalPosts||0);App.totalPages=Math.max(1,Math.ceil(App.totalPosts/Config.pageSize));
      const globalMode=GM_getValue(Config.popularModeKey,'native'),allowedModes=['native','local','aggregate'];App.popularMode=allowedModes.includes(globalMode)?globalMode:allowedModes.includes(saved?.mode)?saved.mode:'native';App.popularAggregatePeriod=PopularPeriod.normalizePeriod(GM_getValue(Config.popularAggregatePeriodKey,saved?.aggregatePeriod||context.period));if(App.popularMode==='aggregate'){const aggregate=await PopularAggregate.load({domain:context.domain,period:App.popularAggregatePeriod});PopularPageController.assertMountCurrent(context,signal);App.popularMeta=aggregate.meta;entries=aggregate.entries;posts=aggregate.posts;App.popularEntries=new Map(entries.map((entry)=>[String(entry.postKey),entry]));App.catalog=new Map([...posts.values()].map((post)=>[String(post.key),post]));await PopularPageController.loadStatuses(context,entries);App.totalPosts=entries.length;App.totalPages=Math.max(1,Math.ceil(App.totalPosts/Config.pageSize));}PopularPageController.assertMountCurrent(context,signal);
    },
    ...
    mountUI(found,context){
      Lifecycle.removeStaleRoots();const root=UI.own(document.createElement('section'));root.id='pmf-popular-root';root.className='pmf-popular-root';root.dataset.popularMode=App.popularMode;root.innerHTML=`<div class="pmf-popular-aggregate-picker" role="group" aria-label="All Scans period" hidden><button type="button" data-pmf-aggregate-period="day" title="Right click for custom date">Day</button><button type="button" data-pmf-aggregate-period="week" title="Right click for custom date">Week</button><button type="button" data-pmf-aggregate-period="month" title="Right click for custom date">Month</button></div><div class="pmf-popular-mode" role="group" aria-label="Popular Posts mode"><button type="button" data-popular-mode="native" class="${App.popularMode==='native'?'pmf-active':''}" aria-pressed="${App.popularMode==='native'}">Native</button><button type="button" data-popular-mode="local" class="${App.popularMode==='local'?'pmf-active':''}" aria-pressed="${App.popularMode==='local'}">Local</button><button type="button" data-popular-mode="aggregate" class="${App.popularMode==='aggregate'?'pmf-active':''}" aria-pressed="${App.popularMode==='aggregate'}">All Scans</button></div><section class="pmf-toolbar pmf-creator-index-toolbar pmf-popular-toolbar"><div class="pmf-controls"><button type="button" class="pmf-filter-button" data-popular-filter></button><button type="button" class="pmf-sort-button" aria-haspopup="menu" aria-expanded="false">Sort: Popular</button><button type="button" class="pmf-scan-button" data-popular-primary></button><button type="button" class="pmf-icon-button" data-popular-settings aria-label="Media Filter settings">${Icons.gear}</button></div><div class="pmf-status"><div class="pmf-status-actions"><button type="button" class="pmf-details-link pmf-link-button" data-popular-queue-toggle>Queue empty</button></div><div class="pmf-status-left" hidden><strong data-popular-status-label></strong></div><div class="pmf-status-right"></div></div><div class="pmf-progress" hidden><span></span></div><section class="pmf-catalogue-details pmf-creator-queue-panel pmf-popular-queue-panel" hidden></section></section>`;
    ...
    bind(){const signal=PopularPageController.controller.signal,ui=App.ui;PopularPageController.layoutSchedule=Util.debounce(()=>{if(PopularPageController.mounted()&&App.popularMode!=='native')PopularPageController.applyLocalLayout();},120);window.addEventListener('resize',PopularPageController.layoutSchedule,{signal});signal.addEventListener('abort',()=>PopularPageController.layoutSchedule?.cancel?.(),{once:true});
      ...
      ui.filterButton.addEventListener('click',(event)=>{event.preventDefault();event.stopPropagation();if(App.popularMode!=='native')UI.toggleFilterPopover();},{signal});
      ui.sortButton.addEventListener('click',(event)=>{event.preventDefault();event.stopPropagation();if(App.popularMode!=='native')UI.toggleSortMenu();},{signal});ui.sortButton.addEventListener('keydown',(event)=>{if(App.popularMode!=='native'&&(event.key==='ArrowDown'||event.key==='ArrowUp')){event.preventDefault();UI.toggleSortMenu(true,event.key==='ArrowUp'?'last':'first');}},{signal});
      ui.queueButton.addEventListener('click',(event)=>{event.preventDefault();event.stopPropagation();PopularPageController.setQueueOpen(!PopularPageController.queueOpen);},{signal});
      ui.root.addEventListener('click',(event)=>{if(event.target.closest('[data-queue-action],[data-queue-tab]')){PopularQueuePanel.handle(event);return;}const aggregatePeriod=event.target.closest('[data-pmf-aggregate-period]')?.dataset.pmfAggregatePeriod;if(aggregatePeriod){PopularPageController.setAggregatePeriod(aggregatePeriod);return;}const mode=event.target.closest('[data-popular-mode]')?.dataset.popularMode;if(mode){PopularPageController.setMode(mode);return;}}, {signal});
      document.addEventListener('click',(event)=>{const link=event.target?.closest?.('a[href]');if(!link||!(App.dom?.periodLinks||[]).includes(link))return;const url=PopularNavigation.normalize(link.getAttribute?.('href')||link.href||'');if(!url||url.searchParams.has('o'))return;if(url.href===location.href)return;event.preventDefault();GM_setValue(Config.popularModeKey,App.popularMode);PopularNavigation.visit(url.href,{reason:'popular-native-period'});},{capture:true,signal});
      document.addEventListener('contextmenu',(event)=>{const control=event.target?.closest?.('[data-pmf-period-type],[data-pmf-aggregate-period]'),period=control?.dataset?.pmfPeriodType||control?.dataset?.pmfAggregatePeriod;if(!period)return;event.preventDefault();event.stopPropagation();PopularDatePicker.open(period,control);},{capture:true,signal});
      const pageClick=(event)=>{const nativeButton=event.target.closest('button[data-native-paginator-index]');if(nativeButton&&!nativeButton.disabled){PopularPageController.navigateNativePage(nativeButton.dataset.nativePaginatorIndex);return;}const button=event.target.closest('button[data-pmf-page-action]');if(button&&!button.disabled)Paginator.activate(button.dataset.pmfPageAction,button.dataset.pmfPage);};ui.paginator.addEventListener('click',pageClick,{signal});ui.paginatorBottom.addEventListener('click',pageClick,{signal});
      ...
      document.addEventListener('keydown',(event)=>{if(App.popularMode==='native'||!['ArrowLeft','ArrowRight'].includes(event.key)||OverlayManager.top()||event.target?.closest?.('input,textarea,select,button,[contenteditable="true"]'))return;const role=event.key==='ArrowRight'?'next':'previous';if(Paginator.activate(role))event.preventDefault();},{capture:true,signal});
    },
    rememberAggregateLink(link){if(!link||PopularPageController.aggregateLinkStates.has(link))return;PopularPageController.aggregateLinkStates.set(link,{hidden:Boolean(link.hidden),display:link.style?.getPropertyValue?.('display')||'',priority:link.style?.getPropertyPriority?.('display')||'',aria:link.getAttribute?.('aria-hidden')??null});},
    stylePeriodLinks(found=App.dom){const controls=new Set(found?.periodLinks||[]);for(const container of found?.navContainers||[])[...container.querySelectorAll?.('a,button,span,strong,b')||[]].forEach((node)=>controls.add(node));for(const link of controls){let url=null;try{const href=link.getAttribute?.('href')||link.href||'';if(href)url=new URL(href,location.origin);}catch{}const label=String(link.textContent||link.getAttribute?.('aria-label')||'').replace(/\s+/g,' ').trim(),exact=/^(day|week|month)$/i.exec(label),navigation=/\b(?:previous|prev|next)\b|[‹›«»←→]/i.test(label);link.classList?.toggle?.('pmf-popular-period-nav-link',navigation);link.classList?.toggle?.('pmf-popular-period-type-link',Boolean(exact));if(exact){const period=PopularPeriod.normalizePeriod(url?.searchParams.get('period')||exact[1]);link.dataset.pmfPeriodType=period;link.title='Right click for custom date';}else{delete link.dataset.pmfPeriodType;if(link.title==='Right click for custom date')link.removeAttribute?.('title');}}},
    restoreAggregatePeriodControls(){for(const[link,state]of PopularPageController.aggregateLinkStates){if(!link?.isConnected)continue;link.hidden=state.hidden;if(state.display)link.style?.setProperty?.('display',state.display,state.priority||'');else link.style?.removeProperty?.('display');if(state.aria==null)link.removeAttribute?.('aria-hidden');else link.setAttribute?.('aria-hidden',state.aria);}PopularPageController.aggregateLinkStates.clear();for(const node of App.dom?.navContainers||[])node.classList?.remove?.('pmf-popular-aggregate-periods');if(App.ui?.aggregatePicker)App.ui.aggregatePicker.hidden=true;const heading=PopularPageController.nativeSnapshot?.heading;if(heading?.node?.isConnected)heading.node.textContent=heading.text;},
    syncAggregatePeriodControls(found=App.dom){PopularPageController.stylePeriodLinks(found);const aggregate=App.popularMode==='aggregate';for(const container of found?.navContainers||[]){if(!PopularDOM.isSafeNativeControlGroup(container,found.grid))continue;PopularPageController.setVisible(container,!aggregate);}if(App.ui?.aggregatePicker){App.ui.aggregatePicker.hidden=!aggregate;App.ui.aggregatePicker.querySelectorAll?.('[data-pmf-aggregate-period]')?.forEach?.((button)=>{const active=button.dataset.pmfAggregatePeriod===App.popularAggregatePeriod;button.classList.toggle('pmf-active',active);button.setAttribute('aria-pressed',String(active));});}const heading=PopularPageController.nativeSnapshot?.heading;if(heading?.node?.isConnected)heading.node.textContent=aggregate?`All Scanned Popular Posts · ${App.popularAggregatePeriod[0].toUpperCase()}${App.popularAggregatePeriod.slice(1)}`:heading.text;},
    refreshStateCurrent(mode,period=App.popularAggregatePeriod){return App.pageKind==='popular'&&PopularPageController.mounted()&&App.popularMode===mode&&(mode!=='aggregate'||App.popularAggregatePeriod===period);},
    async setMode(mode){const next=['native','local','aggregate'].includes(mode)?mode:'native';if(next===App.popularMode){PopularPageController.syncAggregatePeriodControls();App.render();return;}App.popularMode=next;GM_setValue(Config.popularModeKey,next);App.filteredPage=1;App.filteredFirstResultIndex=0;App.filteredAnchorId='';const requestedPeriod=App.popularAggregatePeriod;PopularPageController.dataLoading=true;PopularPageController.syncAggregatePeriodControls();App.render();try{await PopularPageController.refresh();}catch(error){if(!PopularPageController.refreshStateCurrent(next,requestedPeriod))return;PopularPageController.dataLoading=false;Logger.error('Popular mode switch failed.',error);UI.flash(error?.message||'Popular mode could not be opened.');App.render();}},
    async setAggregatePeriod(period){const next=PopularPeriod.normalizePeriod(period);if(next===App.popularAggregatePeriod&&App.popularMode==='aggregate')return;App.popularAggregatePeriod=next;GM_setValue(Config.popularAggregatePeriodKey,next);App.filteredPage=1;App.filteredFirstResultIndex=0;App.filteredAnchorId='';PopularPageController.dataLoading=true;PopularPageController.syncAggregatePeriodControls();App.render();try{await PopularPageController.refresh();}catch(error){if(!PopularPageController.refreshStateCurrent('aggregate',next))return;PopularPageController.dataLoading=false;Logger.error('Popular aggregate period switch failed.',error);UI.flash(error?.message||'The aggregate period could not be opened.');App.render();}},
    nativeCountText(found=App.dom,context=App.popularContext){const offset=Math.max(0,Number(context?.offset)||0),cards=(found?.nativeCards||[]).filter((card)=>card?.isConnected!==false).length,total=Math.max(cards+offset,Number(found?.totalPosts)||0,Number(App.totalPosts)||0);if(!cards&&!total)return'Showing 0 of 0';const start=cards?offset+1:0,end=cards?Math.min(total||offset+cards,offset+cards):0;return`Showing ${start}–${end} of ${Math.max(total,end)}`;},
    setPaginatorMode(mode){const native=mode==='native',ui=App.ui;if(!ui)return;for(const node of [ui.statusFilters,ui.filteredCount,ui.filteredControls,ui.filteredCountBottom,ui.filteredControlsBottom])if(node)node.hidden=native;for(const node of [ui.nativeTopMirror,ui.nativeBottomMirror])if(node)node.hidden=!native;},
    renderNativePaginators(found=App.dom){const ui=App.ui,native=found?.topPaginator;if(!ui||!native)return false;const pending=PopularPageController.nativePagePending,options={includeCount:true,countText:PopularPageController.nativeCountText(found),pendingRole:pending?.role||'',navigationPending:Boolean(pending),omitFirstLast:true,maxPageButtons:10,compactOnMobile:false},top=NativePaginatorMirror.render(native,ui.nativeTopMirror,options),bottom=NativePaginatorMirror.render(native,ui.nativeBottomMirror,options);ui.paginator.hidden=!top;ui.paginatorBottom.hidden=!bottom;return top||bottom;},
    navigateNativePage(index){if(PopularPageController.nativePagePending||App.popularMode!=='native')return false;const candidate=NativePaginatorMirror.items(App.dom?.topPaginator).find((item)=>item.index===Number(index));if(!candidate||candidate.disabled)return false;PopularPageController.nativePagePending={index:candidate.index,role:candidate.role,startedAt:Date.now()};PopularPageController.renderNativePaginators();const activated=NativePaginatorMirror.activate(App.dom.topPaginator,candidate.index);if(!activated){PopularPageController.nativePagePending=null;PopularPageController.renderNativePaginators();return false;}clearTimeout(PopularPageController.nativePageTimer);PopularPageController.nativePageTimer=setTimeout(()=>{if(!PopularPageController.nativePagePending)return;PopularPageController.nativePagePending=null;if(PopularPageController.mounted()){PopularPageController.renderNativePaginators();Lifecycle.routeHealth('popular native paginator timeout');}},4000);PopularPageController.nativePageTimer?.unref?.();return true;},
    ...
    async primaryAction(){const context=App.popularContext;if(!context||App.popularMode==='aggregate')return;const state=PopularPageController.periodState();if(state.busy||(App.popularMode==='native'&&state.complete)||(App.popularMode==='local'&&!state.hasStored))return;const retry=App.popularMode==='native'&&state.incomplete,action=App.popularMode==='native'?(retry?'resume':'scan'):PopularPageController.actionFor(),displayAction=App.popularMode==='native'?(retry?'Retry Scan':'Scan'):'Update',total=Math.max(1,state.expected||Number(App.dom?.totalPosts)||Config.pageSize*10);UI.confirmDialog({title:`${displayAction} Popular Posts?`,paragraphs:[`Period: ${context.label}`,`Posts: up to ${total.toLocaleString()}`],confirmLabel:`Queue ${displayAction.toLowerCase()}`,opener:App.ui.scanButton,onConfirm:()=>{const result=PopularJobManager.enqueue({...context,totalPosts:total,totalKnown:Boolean(state.expected)},action);if(!result.accepted)UI.flash(result.state==='already-queued'?`Already queued · Position ${result.position}`:'This Popular period is already running.');else PopularPageController.setQueueOpen(true);App.render();}});},
    renderQueue(){PopularQueuePanel.render();},
    renderAction(){const aggregate=App.popularMode==='aggregate',job=aggregate?null:PopularJobManager.jobForPeriod(App.popularContext?.periodKey),state=PopularPageController.periodState(App.popularMeta,job),native=App.popularMode==='native',locked=aggregate||!App.popularContext?.periodKey||state.busy||(native?state.complete:!state.hasStored);App.ui.scanButton.disabled=Boolean(locked);App.ui.scanButton.textContent=aggregate?'All scans':native?(state.busy||state.complete?'Scanned':state.incomplete?'Retry scans':'Scan'):'Update';App.ui.scanButton.classList.toggle('pmf-locked-control',Boolean(locked));App.ui.scanButton.classList.remove('pmf-stop-button');App.ui.scanButton.setAttribute('aria-disabled',String(Boolean(locked)));App.ui.progress.hidden=job?.status!=='running';if(job?.status==='running')App.ui.progressBar.style.width=`${PopularQueuePanel.jobPercent(job)}%`;else App.ui.progressBar.style.width='0%';},
    applyNativeControlVisibility(found=App.dom){const ownsUi=(node)=>Boolean(node?.contains?.(PopularPageController.root)||node?.querySelector?.('[data-pmf-owned="true"]'));for(const node of found?.navContainers||[])if(!ownsUi(node)&&PopularDOM.isSafeNativeControlGroup(node,found.grid))PopularPageController.setVisible(node,true);for(const node of found?.paginators||[])if(!ownsUi(node))PopularPageController.setVisible(node,false);for(const node of found?.countNodes||[])if(!ownsUi(node))PopularPageController.setVisible(node,false);for(const node of found?.restrictionNotices||[])if(!ownsUi(node))PopularPageController.setVisible(node,false);PopularPageController.syncAggregatePeriodControls(found);},
    renderNative(){
      PopularPageController.setVisible(App.dom.grid,true);PopularPageController.setVisible(App.ui.grid,false);PopularPageController.applyNativeControlVisibility(App.dom);PopularPageController.setPaginatorMode('native');PopularPageController.renderNativePaginators();App.ui.filterButton.textContent='All posts';App.ui.filterButton.disabled=true;App.ui.filterButton.classList.add('pmf-locked-control');App.ui.sortButton.textContent='Sort: Popular';App.ui.sortButton.disabled=true;App.ui.sortButton.classList.add('pmf-locked-control');App.ui.statusLeft.hidden=true;App.ui.statusLabel.textContent='';App.ui.statusRight.textContent='Native Popular Posts · Pawchive controls';
    },
    renderLocal(){
      PopularPageController.setVisible(App.dom.grid,false);PopularPageController.applyNativeControlVisibility(App.dom);PopularPageController.setVisible(App.ui.grid,true);PopularPageController.setPaginatorMode('local');App.ui.paginator.hidden=false;App.ui.paginatorBottom.hidden=false;App.ui.filterButton.disabled=false;App.ui.filterButton.classList.remove('pmf-locked-control');UI.updateFilterButton();UI.updateQuickStatusFilters();App.ui.sortButton.disabled=false;App.ui.sortButton.classList.remove('pmf-locked-control');UI.updateSortButton();
      ...
      if(PopularPageController.dataLoading){const empty=document.createElement('div');empty.className='pmf-empty';empty.innerHTML='<strong>Loading scanned Popular Posts…</strong>';App.ui.grid.append(empty);}else if(App.popularMode==='aggregate'&&!Number(App.popularMeta?.periodCount)){const empty=document.createElement('div');empty.className='pmf-empty';const label=App.popularAggregatePeriod[0].toUpperCase()+App.popularAggregatePeriod.slice(1);empty.innerHTML=`<strong>No scanned ${label} Popular Posts snapshots.</strong>`;App.ui.grid.append(empty);}else if(App.popularMode==='local'&&!App.popularMeta){const empty=document.createElement('div');empty.className='pmf-empty';empty.innerHTML='<strong>This Popular Posts period has not been scanned.</strong>';App.ui.grid.append(empty);}else if(!pagePosts.length){const empty=document.createElement('div');empty.className='pmf-empty';empty.innerHTML='<strong>No matching posts</strong><span>Adjust the media or status filters.</span>';App.ui.grid.append(empty);}else for(const post of pagePosts){const card=App.dom.template?CardRenderer.clone(post):CardRenderer.fallback(post);App.ui.grid.append(card);PopularCardDecorator.apply(card,post);}PopularPageController.applyLocalLayout();App.ui.statusLeft.hidden=true;App.ui.statusLabel.textContent='';App.ui.statusRight.textContent=App.popularMode==='aggregate'?`All Scans · ${App.popularAggregatePeriod[0].toUpperCase()}${App.popularAggregatePeriod.slice(1)} · ${App.popularEntries.size} unique posts from ${Number(App.popularMeta?.periodCount)||0} snapshots`:`Local Popular Posts · ${App.popularEntries.size} stored`;
    },
    render(){if(!App.ui||App.pageKind!=='popular')return;PmfDomMutationGuard.run(()=>{App.ui.root.dataset.popularMode=App.popularMode;App.ui.root.querySelectorAll('[data-popular-mode]').forEach((button)=>{const active=button.dataset.popularMode===App.popularMode;button.classList.toggle('pmf-active',active);button.setAttribute('aria-pressed',String(active));});if(App.popularMode==='native')PopularPageController.renderNative();else PopularPageController.renderLocal();PopularPageController.renderAction();PopularPageController.renderQueue();});},
    async refresh(){const context=App.popularContext;if(!context)return;const revision=++PopularPageController.refreshRevision,mode=App.popularMode,aggregatePeriod=App.popularAggregatePeriod;PopularPageController.dataLoading=true;App.render();let meta,entries,posts;if(mode==='aggregate'){const aggregate=await PopularAggregate.load({domain:context.domain,period:aggregatePeriod});meta=aggregate.meta;entries=aggregate.entries;posts=aggregate.posts;}else{meta=await Cache.getPopularPeriod(context.periodKey);entries=await Cache.getPopularEntries(context.periodKey);posts=await Cache.getPostsByKeys(entries.map((entry)=>entry.postKey));}if(revision!==PopularPageController.refreshRevision||context.periodKey!==App.popularContext?.periodKey||mode!==App.popularMode||(mode==='aggregate'&&aggregatePeriod!==App.popularAggregatePeriod))return;App.popularMeta=meta;App.popularEntries=new Map(entries.map((entry)=>[String(entry.postKey),entry]));App.catalog=new Map([...posts.values()].map((post)=>[String(post.key),post]));await PopularPageController.loadStatuses(context,entries);if(revision!==PopularPageController.refreshRevision)return;App.totalPosts=mode==='aggregate'?entries.length:Math.max(Number(meta?.totalExpectedPosts)||0,entries.length,App.dom?.totalPosts||0);App.totalPages=Math.max(1,Math.ceil(App.totalPosts/Config.pageSize));PopularPageController.dataLoading=false;PopularPageController.syncAggregatePeriodControls();App.render();},
    rebindNative(found,page=Route.parsePage(location.href)){
      if(PopularPageController.rebinding||!found?.grid||page.kind!=='popular')return false;const context=PopularPeriod.fromPage(page,found);if(!context.periodKey||context.pageKey!==App.popularContext?.pageKey)return false;PopularPageController.rebinding=true;try{PopularPageController.restoreNative();PopularPageController.nativeSnapshot=PopularPageController.saveNative(found);PopularPageController.found=found;App.dom={...App.dom,...found,countNodes:found.countNodes||[],restrictionNotices:found.restrictionNotices||[],topPaginator:found.topPaginator,bottomPaginator:found.bottomPaginator};App.dom.template=found.template||App.dom.template;App.dom.totalPosts=found.totalPosts||App.dom.totalPosts;PopularPageController.applyLocalLayout(found);PopularPageController.nativePagePending=null;clearTimeout(PopularPageController.nativePageTimer);PopularPageController.nativePageTimer=null;PopularPageController.applyNativeControlVisibility(found);App.render();return true;}finally{PopularPageController.rebinding=false;}
    },
  ...
  const App = {
    ...
    setSort(mode){
      const sorter=App.pageKind==='popular'?PopularSorter:PostSorter,next=sorter.nextSelection(App.sortMode,App.sortDirection,mode);App.sortMode=next.mode;App.sortDirection=next.direction;App.filteredPage=1;App.filteredFirstResultIndex=0;App.filteredAnchorId='';App.persistUIState();UI.updateSortButton();App.render();return next;
    },
    ...
    matchingPosts() { const matches=App.cataloguePosts().filter((post)=>FilterEngine.matches(post,App.filterState,App.query,App.statusFor(post)));return App.pageKind==='popular'?PopularSorter.sort(matches,App.popularEntries,{mode:App.sortMode,direction:App.sortDirection}):PostSorter.sort(matches,{mode:App.sortMode,direction:App.sortDirection}); },
    ...
    uiStateRecord() {
      ...
      return App.pageKind==='popular'?{...common,periodKey:App.popularContext?.periodKey,mode:App.popularMode,aggregatePeriod:App.popularAggregatePeriod}:{...common,creatorKey:App.context.creatorKey};
    },
  ...
  GM_addStyle(`
    ...
    .pmf-maintenance-center{width:min(720px,calc(100vw - 28px));max-height:min(820px,calc(100vh - 28px))}.pmf-maintenance-center-body{display:flex;flex-direction:column;gap:12px}.pmf-maintenance-center .pmf-settings-section{margin:0}.pmf-maintenance-center .pmf-settings-actions{margin-top:8px}.pmf-maintenance-danger{border-color:#6d3530!important}
    [data-pmf-creator-service-badge="fanbox"]{background:#30343a!important;background-image:none!important;color:#f4f5f6!important;border-color:#565d63!important;box-shadow:none!important}
  ...
  GM_addStyle(`
    ...
    .pmf-creator-rule-row>*{min-width:0}.pmf-creator-rule-condition{display:grid;grid-template-columns:minmax(180px,260px);gap:10px;align-items:end}.pmf-creator-rule-controls{display:grid;grid-template-columns:68px 92px minmax(150px,1.2fr) minmax(155px,1fr) 106px minmax(145px,1fr) minmax(76px,88px) 14px minmax(76px,88px) 30px;gap:6px;align-items:center;min-width:0}
    .pmf-creator-rule-row .pmf-rule-fields{overflow:hidden;text-overflow:ellipsis;white-space:nowrap;text-align:left}
    .pmf-creator-rule-row button[data-creator-rule-action="remove"]{width:30px!important;min-width:30px!important;padding:0!important;color:#ffaaa0}
    ...
    @media(max-width:1100px){.pmf-creator-rule-controls{grid-template-columns:68px 88px minmax(150px,1fr) minmax(145px,1fr) 100px minmax(135px,1fr) minmax(72px,82px) 14px minmax(72px,82px) 30px;min-width:900px}.pmf-creator-rules-dialog .pmf-editor-body{overflow-x:auto}}
  ...
  GM_addStyle(`
    @media(max-width:760px){
      ...
      .pmf-creator-filter-popover{width:calc(100vw - 16px)!important;max-width:calc(100vw - 16px)!important;padding:10px}
      .pmf-creator-filter-popover .pmf-filter-row{grid-template-columns:minmax(0,1fr) 42px;min-height:42px}
      .pmf-creator-filter-popover .pmf-filter-row>label{min-height:42px;padding:5px 2px;line-height:1.3}
      .pmf-creator-filter-popover .pmf-row-chevron{width:42px;min-height:40px!important;font-size:24px}
      .pmf-creator-partial-option{min-height:42px;margin-top:6px;padding:6px 2px;line-height:1.35}
      .pmf-page-controls{max-width:100%;flex-wrap:wrap;gap:6px}
      .pmf-page-controls button{min-width:38px}
      .pmf-catalogue-details,.pmf-scan-error-details{margin-left:8px;margin-right:8px;padding:10px}
      .pmf-popular-mode-selector,.pmf-popular-aggregate-picker{width:min(100%,420px)}
    ...
    @media(max-width:430px){
      ...
      .pmf-reconstructed-creator-content{padding-left:86px;padding-right:48px}
      .pmf-reconstructed-creator-visual img,.pmf-reconstructed-avatar-placeholder{width:62px;height:62px}
      .pmf-page-controls button{min-width:36px;height:34px;padding-left:6px;padding-right:6px}
  ...
  GM_addStyle(`
    ...
    .pmf-popular-aggregate-picker{display:grid;grid-template-columns:repeat(3,minmax(58px,1fr));align-items:center;justify-items:center;width:min(228px,calc(100% - 16px));margin:0 auto 6px}.pmf-popular-aggregate-picker[hidden]{display:none!important}.pmf-popular-aggregate-picker button{padding:1px 4px;border:0;background:none;color:#e6b096;font:inherit;cursor:pointer}.pmf-popular-aggregate-picker button.pmf-active{color:#ffd1b7;font-weight:700;text-decoration:underline;text-underline-offset:3px}
    .pmf-popular-mode{display:grid;grid-template-columns:repeat(3,1fr);width:min(480px,calc(100% - 16px));margin:0 auto 8px;font:inherit}
    ...
    .pmf-popular-mode button+button{border-left:0}.pmf-popular-mode button:first-child{border-radius:4px 0 0 4px}.pmf-popular-mode button:last-child{border-radius:0 4px 4px 0}
    .pmf-popular-mode button.pmf-active{color:#ffc5a3;background:#3b2b24;box-shadow:inset 0 0 0 1px #8e624d}
    .pmf-popular-period-nav-link,.pmf-popular-period-nav-link:link,.pmf-popular-period-nav-link:visited,.pmf-popular-period-nav-link:hover,.pmf-popular-period-nav-link:focus,.pmf-popular-period-nav-link:active{color:#e6b096!important;font-weight:400!important;text-decoration:none!important}.pmf-popular-period-type-link{cursor:pointer}.pmf-popular-date-dialog .pmf-help{margin:8px 0 0;color:var(--pmf-muted);font-size:12px}
  ...
  const Lifecycle = {
    ...
    historySignalInstalled:false,canonicalRedirecting:false,
    ...
    redirectCanonicalPopular(){const target=Route.canonicalPopularUrl(location.href);if(!target)return false;if(Lifecycle.canonicalRedirecting)return true;Lifecycle.canonicalRedirecting=true;Logger.info({operation:'popular-canonical-redirect',from:location.href,to:target});try{if(typeof location.replace==='function')location.replace(target);else location.href=target;}catch{location.href=target;}return true;},
    schedule(reason = 'signal', delay = 0) {
      if (Lifecycle.stopped||Lifecycle.redirectCanonicalPopular()) return;
    ...
    prepareSnapshot(reason = 'pagehide',event=null) {
      Logger.info('Preparing page snapshot:', reason);App.savePresetDebounced.cancel(); App.saveActivePreset(); App.persistUIState();CreatorSessionCache.captureFromApp();if(reason==='turbo:before-cache'){Lifecycle.mountController?.abort();Lifecycle.mountingPageKey='';Lifecycle.mountPromise=null;CatalogueJobManager.suspendForBfcache();PopularJobManager.persist();Lifecycle.documentObserver?.disconnect();Logger.info({operation:'soft-snapshot-turbo',pageKey:Lifecycle.activePageKey,popularQueueActive:Boolean(PopularJobManager.activeJob),popularQueueWaiting:PopularJobManager.pendingJobs.length});return;}if(event?.persisted){Lifecycle.mountController?.abort();Lifecycle.mountingPageKey='';Lifecycle.mountPromise=null;CatalogueJobManager.suspendForBfcache();PopularJobManager.suspendForBfcache();Lifecycle.documentObserver?.disconnect();Logger.info({operation:'soft-snapshot-bfcache',pageKey:Lifecycle.activePageKey,rootConnected:Boolean(App.ui?.root?.isConnected),nativeAnchorConnected:Boolean(App.dom?.grid?.isConnected||PostPageController.nativeFavorite?.isConnected)});return;}CatalogueJobManager.shutdown();PopularJobManager.shutdown(); App.detachPage();PostPageController.cleanup();ArtistsPageController.cleanup();PopularPageController.cleanup();Lifecycle.restoreNativePage(); Lifecycle.removeStaleRoots();
    },
    ...
    start() {
      if(Lifecycle.redirectCanonicalPopular())return;
