...
(function () {
  ...
  const PopularDOM = {
    ...
    paginationGroupFor(link,{root=document,main=null,grid=null}={}){
      ...
      while(candidate&&candidate!==main&&candidate!==boundary){
        if(candidate===grid||candidate.contains?.(grid)||PopularDOM.nativeCardCandidates(candidate).length)return null;
    ...
    paginatorCandidates(root=document,{main=null,grid=null}={}){
      ...
      return [...new Set(values)].filter((node)=>!PopularDOM.isOwned(node)&&node!==grid&&!node.contains?.(grid)&&!PopularDOM.nativeCardCandidates(node).length&&node.querySelector?.('a,button,li'));
    },
    find(page={}){
      ...
      const paginators=PopularDOM.paginatorCandidates(document,{main,grid});const links=PopularDOM.pageLinkCandidates(document);
  ...
  const PopularQueuePanel = {
    ...
    row(job,kind){const progress=job.progress||{},periodKey=Util.escapeHtml(job.periodKey||''),actions=kind==='active'?'<button data-popular-queue-action="stop">Stop</button>':kind==='waiting'?'<button data-popular-queue-action="top">Move to top</button><button data-popular-queue-action="remove">Remove</button>':kind==='complete'?'':'<button data-popular-queue-action="retry">Retry</button><button data-popular-queue-action="dismiss">Dismiss</button>';return`<div class="pmf-queue-row" data-period-key="${periodKey}"><div><strong>${Util.escapeHtml(PopularPeriod.label(job.context))}</strong><small>${Util.escapeHtml(progress.message||`${job.requestedAction||'scan'} · ${job.status}`)}</small>${kind==='active'?`<progress max="100" value="${PopularQueuePanel.progress(job)}" aria-label="${Util.escapeHtml(PopularPeriod.label(job.context))} progress"></progress>`:''}</div><span>${actions}</span></div>`;},
    render(snapshot=PopularJobManager.snapshot()){
      const host=App.ui?.queue,toggle=App.ui?.queueButton;if(!host||!toggle)return;const selectedTab=host.querySelector('[data-popular-queue-tab].pmf-active')?.dataset.popularQueueTab||host.dataset.selectedTab||'queue',scrollTop=host.scrollTop,openDetails=new Set([...host.querySelectorAll('details[open][data-detail-key]')].map((item)=>item.dataset.detailKey));const active=snapshot.active||[],waiting=snapshot.pending||[],issues=snapshot.issues||[],recent=snapshot.recent||[],complete=recent.filter((job)=>job.status==='complete'),overall=PopularQueuePanel.aggregate(snapshot),percent=overall.total?Math.round(overall.finished/overall.total*100):0,retained=Boolean(active.length||waiting.length||recent.length||issues.length);
      toggle.textContent=active.length||waiting.length?`${active.length} active · ${waiting.length} waiting`:recent.length?`${recent.length} recent job${recent.length===1?'':'s'}`:'Queue empty';host.hidden=!PopularPageController.queueOpen;
      host.innerHTML=`<div class="pmf-queue-tabs"><button data-popular-queue-tab="queue" class="pmf-active">Queue</button><button data-popular-queue-tab="issues">Issues${issues.length?` (${issues.length})`:''}</button></div><div data-popular-queue-pane="queue"><section class="pmf-queue-progress"><strong>Overall batch progress</strong><span>${overall.finished} of ${overall.total} finished · ${overall.remaining} remaining · ${overall.active} active</span><progress max="100" value="${percent}" aria-label="${overall.finished} of ${overall.total} Popular operations finished"></progress><small>Concurrency: 1</small></section>${active.length?`<h3>Active</h3>${active.map((job)=>PopularQueuePanel.row(job,'active')).join('')}`:''}${waiting.length?`<h3>Waiting</h3>${waiting.map((job)=>PopularQueuePanel.row(job,'waiting')).join('')}`:''}${complete.length?`<details data-detail-key="recent-completed"><summary>Recently completed (${complete.length})</summary>${complete.map((job)=>PopularQueuePanel.row(job,'complete')).join('')}</details><button data-popular-queue-action="clear-completed">Clear completed</button>`:''}${!retained?'<p class="pmf-help">Queue empty</p>':!overall.remaining&&!active.length?'<p class="pmf-help">Queue idle</p>':''}</div><div data-popular-queue-pane="issues" hidden>${issues.length?issues.map((job)=>PopularQueuePanel.row(job,'issue')).join(''):'<p class="pmf-help">No queue issues.</p>'}</div>`;
      host.querySelectorAll('[data-popular-queue-tab]').forEach((button)=>button.classList.toggle('pmf-active',button.dataset.popularQueueTab===selectedTab));host.querySelectorAll('[data-popular-queue-pane]').forEach((pane)=>pane.hidden=pane.dataset.popularQueuePane!==selectedTab);host.dataset.selectedTab=selectedTab;openDetails.forEach((key)=>{const details=host.querySelector(`details[data-detail-key="${key}"]`);if(details)details.open=true;});host.scrollTop=scrollTop;
    },
    handle(event){const tab=event.target.closest('[data-popular-queue-tab]')?.dataset.popularQueueTab;if(tab){App.ui.queue.querySelectorAll('[data-popular-queue-tab]').forEach((button)=>button.classList.toggle('pmf-active',button.dataset.popularQueueTab===tab));App.ui.queue.querySelectorAll('[data-popular-queue-pane]').forEach((pane)=>pane.hidden=pane.dataset.popularQueuePane!==tab);return true;}const action=event.target.closest('[data-popular-queue-action]')?.dataset.popularQueueAction;if(!action)return false;const key=event.target.closest('[data-period-key]')?.dataset.periodKey;if(action==='stop')PopularJobManager.stop(key);if(action==='top')PopularJobManager.moveToTop(key);if(action==='remove')PopularJobManager.remove(key);if(action==='retry')PopularJobManager.retry(key);if(action==='dismiss')PopularJobManager.dismiss(key);if(action==='clear-completed')PopularJobManager.clearCompleted();PopularQueuePanel.render();return true;},
  };
...
UI.closeSettings('reopen');const checked=(value)=>value?'checked':'';const selected=(value,current)=>value===current?'selected':'';const backdrop=document.createElement('div');backdrop.className='pmf-modal-backdrop';App.ui.settingsBadgeTypes=Util.clone(Settings.value.catalogueBadges.types);App.ui.settingsCreatorBadgeTypes=Util.clone(Settings.value.creatorCardBadges.types);App.ui.settingsStatusBadgeTypes=Util.clone(Settings.value.postStatusBadges.types);App.ui.settingsPreview={displayMode:App.displayMode,compactCardScale:Settings.value.compactCardScale,compactThumbnailAspectRatio:Settings.value.compactThumbnailAspectRatio,postAttachmentBadgeSize:Settings.value.postAttachmentBadgeSize,filteredPage:App.filteredPage,postStatusBadgeSize:Settings.value.postStatusBadgeSize,seenCardTreatment:Util.clone(Settings.value.seenCardTreatment),committed:false};
  ...
  const PopularCardDecorator = {
    platformIcon(card,post){const footer=card.querySelector?.('.post-card__footer');const service=String(post?.service||card.dataset?.service||'').toLowerCase();const candidate=[...footer?.querySelectorAll?.('img')||[]].find((image)=>{const source=String(image.getAttribute?.('src')||image.src||'').toLowerCase();return /small_icons\/(?:fanbox|patreon)\.png/.test(source)||service&&source.includes(`/${service}.`);});const icon=candidate?.cloneNode?.(true)||document.createElement('img');if(!candidate&&['fanbox','patreon'].includes(service))icon.src=`/static/small_icons/${service}.png`;if(!icon.getAttribute?.('src')&&!icon.src)return null;icon.removeAttribute?.('width');icon.removeAttribute?.('height');icon.className='pmf-popular-platform-icon';icon.dataset.pmfOwned='true';icon.dataset.pmfInstance=INSTANCE_ID;icon.alt=service==='fanbox'?'Pixiv Fanbox':service==='patreon'?'Patreon':'Post platform';icon.title=icon.alt;return icon;},
  ...
  const PopularPageController = {
    ...
    periodLinkDescriptor(link){try{const url=new URL(link.href||link.getAttribute?.('href'),location.origin);if(url.pathname.replace(/\/+$/,'').toLowerCase()!=='/posts/popular')return null;const label=String(link.textContent||'').trim().replace(/\s+/g,' ');const rel=String(link.getAttribute?.('rel')||'').toLowerCase();const period=PopularPeriod.normalizePeriod(url.searchParams.get('period'));const date=PopularPeriod.normalizeDate(url.searchParams.get('date'));let direction='';if(rel==='prev'||/previous|prev|[‹«←]/i.test(label))direction='previous';else if(rel==='next'||/next|[›»→]/i.test(label))direction='next';return{href:url.href,label,period,date,direction};}catch{return null;}},
    periodNavigationModel(found,context){
      const descriptors=(found.periodLinks||[]).map((link)=>PopularPageController.periodLinkDescriptor(link)).filter(Boolean);const currentPeriod=PopularPeriod.normalizePeriod(context.period);const samePeriod=descriptors.filter((item)=>item.period===currentPeriod);const previous=samePeriod.find((item)=>item.direction==='previous')||null;const next=samePeriod.find((item)=>item.direction==='next')||null;const periodButtons=['day','week','month'].map((period)=>{const candidates=descriptors.filter((item)=>!item.direction&&item.period===period);const exact=candidates.find((item)=>item.date===context.date)||candidates[0];return{role:period,label:period[0].toUpperCase()+period.slice(1),href:exact?.href||PopularPeriod.pageUrl({...context,period,date:context.date},0),disabled:false,current:period===currentPeriod};});return[{role:'previous',label:'‹ prev',href:previous?.href||'',disabled:!previous,current:false},...periodButtons,{role:'next',label:'next ›',href:next?.href||'',disabled:!next,current:false}];
    },
    periodNavMarkup(found,context){const values=PopularPageController.periodNavigationModel(found,context);return `<div class="pmf-popular-period-title">${Util.escapeHtml(context.label)}</div><nav class="pmf-popular-period-links" aria-label="Popular Posts period navigation">${values.map(({href,label,role,disabled,current})=>`<button type="button" data-popular-period-role="${role}" data-popular-period-url="${Util.escapeHtml(href)}" ${disabled?'disabled aria-disabled="true"':''} ${current?'aria-current="page"':''}>${Util.escapeHtml(label)}</button>`).join('')}</nav>`;},
    navigatePeriod(button){if(!button||button.disabled||button.getAttribute('aria-disabled')==='true')return false;const href=button.dataset.popularPeriodUrl;if(!href)return false;return PopularNavigation.visit(href,{reason:'popular-period'});},
    ...
    bind(){const signal=PopularPageController.controller.signal,ui=App.ui;PopularPageController.layoutSchedule=Util.debounce(()=>{if(PopularPageController.mounted()&&App.popularMode==='local')PopularPageController.applyLocalLayout();},120);window.addEventListener('resize',PopularPageController.layoutSchedule,{signal});signal.addEventListener('abort',()=>PopularPageController.layoutSchedule?.cancel?.(),{once:true});
      ...
      ui.root.addEventListener('click',(event)=>{if(event.target.closest('[data-popular-queue-action],[data-popular-queue-tab]')){PopularQueuePanel.handle(event);return;}const periodButton=event.target.closest('[data-popular-period-url]');if(periodButton){event.preventDefault();PopularPageController.navigatePeriod(periodButton);return;}const mode=event.target.closest('[data-popular-mode]')?.dataset.popularMode;if(mode){App.popularMode=mode;GM_setValue(Config.popularModeKey,mode);App.filteredPage=1;App.persistUIState();App.render();return;}}, {signal});
    ...
    renderLocal(){
      ...
      if(!App.popularMeta){const empty=document.createElement('div');empty.className='pmf-empty';empty.innerHTML='<strong>This Popular Posts period has not been scanned.</strong><span>Use Scan to queue this period and build its Local snapshot.</span>';App.ui.grid.append(empty);}else if(!pagePosts.length){const empty=document.createElement('div');empty.className='pmf-empty';empty.innerHTML='<strong>No matching posts</strong><span>Adjust the media or status filters.</span>';App.ui.grid.append(empty);}else for(const post of pagePosts){const card=App.dom.template?CardRenderer.clone(post):CardRenderer.fallback(post);App.ui.grid.append(card);PopularCardDecorator.apply(card,post);}PopularPageController.applyLocalLayout();App.ui.statusLeft.hidden=false;App.ui.statusLabel.textContent=`${matches.length} Local Popular posts`;App.ui.statusRight.textContent=`Local Popular Posts · ${App.popularEntries.size} stored`;
    },
  ...
  GM_addStyle(`
    ...
    .pmf-popular-period{width:max-content;max-width:calc(100% - 24px);padding:5px 10px;margin:0 auto 7px;text-align:center;background:linear-gradient(180deg,#1f2326e8,#171a1de8)}
    .pmf-popular-period-title{margin-bottom:3px;color:#f1d0c0;font-size:11px;line-height:1.15;font-weight:600}
    .pmf-popular-period-links{display:flex;flex-wrap:nowrap;justify-content:center;align-items:center;gap:7px}
    .pmf-popular-period-links button{appearance:none;padding:0;border:0;background:none;color:#efaaa4;font:inherit;font-size:11px;line-height:1.15;white-space:nowrap;cursor:pointer}
    .pmf-popular-period-links button:hover:not(:disabled){text-decoration:underline}.pmf-popular-period-links button[aria-current="page"]{color:#ffd0b4;font-weight:700;text-decoration:underline}.pmf-popular-period-links button[data-popular-period-role="previous"],.pmf-popular-period-links button[data-popular-period-role="next"]{color:#f4c6b0}.pmf-popular-period-links button:disabled{color:#73797e!important;text-decoration:line-through;cursor:not-allowed;opacity:.7}
    ...
    .pmf-popular-toolbar .pmf-status{display:grid;grid-template-columns:auto minmax(0,1fr) auto;column-gap:12px;align-items:center}.pmf-popular-toolbar .pmf-status-left{display:flex;align-items:center;gap:10px;min-width:0}.pmf-popular-toolbar .pmf-status-left strong{overflow:hidden;text-overflow:ellipsis;white-space:nowrap}.pmf-popular-toolbar .pmf-status-actions{margin-left:0;min-width:max-content}.pmf-popular-toolbar .pmf-status-right{margin-left:0;justify-self:end}
    ...
    .pmf-popular-queue-panel{border-top:1px solid var(--pmf-border);padding:10px 12px;background:#15181b}
    ...
    .pmf-popular-footer-meta{display:flex;flex-direction:column;min-width:0}.pmf-popular-footer-meta time{display:block;min-width:0;overflow:hidden;text-overflow:ellipsis;white-space:nowrap}.pmf-popular-attachment-source{font-size:0;line-height:0}
    .pmf-empty-inline{padding:8px;color:var(--pmf-muted);text-align:center}
    @media(max-width:760px){
      #pmf-popular-root{width:100%}
      .pmf-popular-period{max-width:calc(100% - 10px)}
      .pmf-popular-period-links{gap:6px}.pmf-popular-period-links button{font-size:11px}
      .pmf-popular-toolbar .pmf-status{grid-template-columns:1fr auto}.pmf-popular-toolbar .pmf-status-left{grid-column:1}.pmf-popular-toolbar .pmf-status-actions{grid-column:1}.pmf-popular-toolbar .pmf-status-right{grid-column:2;grid-row:1/3;align-self:center}
