...
(function () {
  ...
  const PopularDOM = {
    ...
    countCandidates(root=document,{paginators=[]}={}){
      const values=[],seen=new Set(),add=(node)=>{if(!node||seen.has(node)||PopularDOM.isOwned(node))return;const text=String(node.textContent||'').replace(/\s+/g,' ').trim();if(!/^showing\s+[\d,]+\s*[-–]\s*[\d,]+\s+of\s+[\d,]+$/i.test(text))return;if(PopularDOM.nativeCardCandidates(node).length)return;seen.add(node);values.push(node);};
      for(const paginator of paginators){[...paginator.querySelectorAll?.('small,[data-count],[class*="count"]')||[]].forEach(add);const parent=paginator.parentElement;if(parent&&parent!==document.body)[...parent.children||[]].forEach((child)=>{if(child!==paginator)add(child);});}
      [...root.querySelectorAll?.('.paginator>small,.paginator>[data-count],.paginator>[class*="count"],small')||[]].forEach(add);return values;
    },
    find(page={}){
...
UI.closeSettings('reopen');const checked=(value)=>value?'checked':'';const selected=(value,current)=>value===current?'selected':'';const backdrop=document.createElement('div');backdrop.className='pmf-modal-backdrop';App.ui.settingsBadgeTypes=Util.clone(Settings.value.catalogueBadges.types);App.ui.settingsCreatorBadgeTypes=Util.clone(Settings.value.creatorCardBadges.types);App.ui.settingsStatusBadgeTypes=Util.clone(Settings.value.postStatusBadges.types);App.ui.settingsPreview={displayMode:App.displayMode,compactCardScale:Settings.value.compactCardScale,compactThumbnailAspectRatio:Settings.value.compactThumbnailAspectRatio,postAttachmentBadgeSize:Settings.value.postAttachmentBadgeSize,filteredPage:App.filteredPage,postStatusBadgeSize:Settings.value.postStatusBadgeSize,seenCardTreatment:Util.clone(Settings.value.seenCardTreatment),committed:false};
  ...
  const PopularPageController = {
    ...
    hasStoredPeriod(meta=App.popularMeta,job=PopularJobManager.jobForPeriod(App.popularContext?.periodKey)){const stored=Math.max(0,Number(meta?.storedEntryCount)||0,Number(App.popularEntries?.size)||0);return Boolean(stored||meta?.completedRunId||meta?.completedAt||job?.status==='complete');},
    async primaryAction(){const context=App.popularContext;if(!context)return;const job=PopularJobManager.jobForPeriod(context.periodKey),busy=Boolean(job&&['queued','running'].includes(job.status)),stored=PopularPageController.hasStoredPeriod(App.popularMeta,job);if(busy||(App.popularMode==='native'&&stored)||(App.popularMode==='local'&&!stored))return;const action=App.popularMode==='native'?'scan':PopularPageController.actionFor(),displayAction=App.popularMode==='native'?'Scan':'Update',total=Math.max(1,Number(App.dom?.totalPosts)||Number(App.popularMeta?.totalExpectedPosts)||500);UI.confirmDialog({title:`${displayAction} Popular Posts?`,paragraphs:[`Period: ${context.label}`,`Posts: up to ${total.toLocaleString()}`],confirmLabel:`Queue ${displayAction.toLowerCase()}`,opener:App.ui.scanButton,onConfirm:()=>{const result=PopularJobManager.enqueue({...context,totalPosts:total,totalKnown:Boolean(Number(App.dom?.totalPosts)||Number(App.popularMeta?.totalExpectedPosts))},action);if(!result.accepted)UI.flash(result.state==='already-queued'?`Already queued · Position ${result.position}`:'This Popular period is already running.');else PopularPageController.queueOpen=true;App.render();}});},
    renderQueue(){PopularQueuePanel.render();},
    renderAction(){const job=PopularJobManager.jobForPeriod(App.popularContext?.periodKey),busy=Boolean(job&&['running','queued'].includes(job.status)),stored=PopularPageController.hasStoredPeriod(App.popularMeta,job),native=App.popularMode==='native',locked=!App.popularContext?.periodKey||(native?(stored||busy):(!stored||busy));App.ui.scanButton.disabled=Boolean(locked);App.ui.scanButton.textContent=native?(stored||busy?'Scanned':'Scan'):'Update';App.ui.scanButton.classList.toggle('pmf-locked-control',Boolean(locked));App.ui.scanButton.classList.remove('pmf-stop-button');App.ui.scanButton.setAttribute('aria-disabled',String(Boolean(locked)));App.ui.progress.hidden=job?.status!=='running';if(job?.status==='running'){const completed=Number(job.progress?.completed)||Number(job.progress?.detailCompleted)||0,total=Number(job.progress?.total)||Number(job.progress?.detailTotal)||App.totalPosts||1;App.ui.progressBar.style.width=`${Math.min(100,completed/Math.max(1,total)*100)}%`;}},
