...
(function () {
  ...
  const CreatorGridGeometry = {
    ...
    measure(grid) {
      const cards=ArtistsDOM.creatorCards({grid},{nativeOnly:true}).map((info)=>Math.round(info.card.getBoundingClientRect?.().width||0)).filter((width)=>width>=120);
      const style=globalThis.getComputedStyle?.(grid);return{cardWidth:Util.clamp(CreatorGridGeometry.median(cards),180,520),columnGap:Util.clamp(parseFloat(style?.columnGap)||8,0,40),rowGap:Util.clamp(parseFloat(style?.rowGap)||8,0,40)};
    },
    apply(grid,geometry={}){if(!grid)return;grid.style.setProperty('--pmf-native-creator-card-width',`${Util.clamp(Number(geometry.cardWidth)||310,180,520)}px`);grid.style.columnGap=`${Util.clamp(Number(geometry.columnGap)||8,0,40)}px`;grid.style.rowGap=`${Util.clamp(Number(geometry.rowGap)||8,0,40)}px`;},
  };
  ...
  const NativeArtistsVisibility = {
    ...
    capture(found,searchInput=null){const elements=[found?.grid,...NativeArtistsVisibility.duplicateElements(found,searchInput)].filter(Boolean);return{grid:found?.grid||null,elements:elements.map((element)=>({element,hidden:Boolean(element.hidden),display:element.style.getPropertyValue('display'),displayPriority:element.style.getPropertyPriority('display'),ariaHidden:element.getAttribute('aria-hidden')})),searchInput,searchPlaceholder:searchInput?.placeholder||''};},
    hide(snapshot){snapshot?.elements?.forEach(({element})=>{if(!element?.isConnected)return;element.hidden=true;element.style.setProperty('display','none','important');element.setAttribute('aria-hidden','true');element.dataset.pmfNativeHidden='true';});},
...
UI.closeSettings('reopen');const checked=(value)=>value?'checked':'';const selected=(value,current)=>value===current?'selected':'';const backdrop=document.createElement('div');backdrop.className='pmf-modal-backdrop';App.ui.settingsBadgeTypes=Util.clone(Settings.value.catalogueBadges.types);App.ui.settingsCreatorBadgeTypes=Util.clone(Settings.value.creatorCardBadges.types);App.ui.settingsStatusBadgeTypes=Util.clone(Settings.value.postStatusBadges.types);App.ui.settingsPreview={displayMode:App.displayMode,compactCardScale:Settings.value.compactCardScale,compactThumbnailAspectRatio:Settings.value.compactThumbnailAspectRatio,postAttachmentBadgeSize:Settings.value.postAttachmentBadgeSize,filteredPage:App.filteredPage,postStatusBadgeSize:Settings.value.postStatusBadgeSize,seenCardTreatment:Util.clone(Settings.value.seenCardTreatment),committed:false};
  ...
  GM_addStyle(`
    ...
    .pmf-creator-index-grid{display:grid!important;grid-template-columns:repeat(auto-fit,minmax(var(--pmf-native-creator-card-width,310px),1fr));gap:8px;width:100%;align-items:stretch}
    ...
    .pmf-reconstructed-creator-card{position:relative;min-height:104px;overflow:hidden;border:1px solid var(--pmf-border);border-radius:8px;background:#1a1f22}.pmf-reconstructed-creator-link{display:block;min-height:104px;color:inherit;text-decoration:none}.pmf-reconstructed-creator-visual{position:absolute;inset:0;display:flex;align-items:center;padding:12px;background-size:cover;background-position:center;background-color:#252b2f}.pmf-reconstructed-creator-visual:after{content:"";position:absolute;inset:0;background:linear-gradient(90deg,#101416e8 0%,#101416a8 55%,#10141678)}.pmf-reconstructed-creator-visual img,.pmf-reconstructed-avatar-placeholder{position:relative;z-index:1;width:76px;height:76px;object-fit:cover;border-radius:7px;background:#111;display:grid;place-items:center;font-size:30px}.pmf-reconstructed-creator-content{position:relative;z-index:2;display:flex;min-height:104px;box-sizing:border-box;flex-direction:column;justify-content:center;gap:4px;padding:12px 64px 12px 102px}.pmf-reconstructed-creator-content strong{font-size:20px;line-height:1.1}.pmf-reconstructed-creator-content small{color:#d8d8d8}
  ...
  const ArtistsPageController = {
    ...
    async mount(page,{generation:routeGeneration=Lifecycle.routeGeneration,signal:routeSignal}={}) {
      ...
      CreatorIndexUI.grid.addEventListener('contextmenu',(event)=>ArtistsPageController.handleContextMenu(event),{signal});CreatorIndexUI.grid.addEventListener('keydown',(event)=>{if(event.key==='ContextMenu'||event.shiftKey&&event.key==='F10'){const card=event.target.closest?.('[data-pmf-creator-key]');if(card&&ArtistsPageController.cardByElement.has(card)){event.preventDefault();ArtistsPageController.openAction(ArtistsPageController.cardByElement.get(card));}}},{signal});
