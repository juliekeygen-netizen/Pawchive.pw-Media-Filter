...
(function () {
  ...
  const NativeArtistsProxy = {
    cloneOptions(source,target,{service=false}={}){target.replaceChildren(...[...source?.options||[]].map((option)=>{const copy=document.createElement('option');copy.value=option.value;copy.textContent=service&&option.value===''?'Any service':option.textContent;copy.selected=option.selected;return copy;}));target.value=source?.value||'';},
    sync(found,host){
      if(!found||!host)return false;const service=host.querySelector('[data-native-proxy="service"]'),sort=host.querySelector('[data-native-proxy="sort"]'),direction=host.querySelector('[data-native-proxy="direction"]');if(!service||!sort||!direction||!found.serviceControl||!found.sortControl||!found.directionControl)return false;NativeArtistsProxy.cloneOptions(found.serviceControl,service,{service:true});NativeArtistsProxy.cloneOptions(found.sortControl,sort);direction.dataset.value=found.directionControl.value||'desc';direction.setAttribute('aria-label',found.directionControl.getAttribute('aria-label')||'Toggle sort direction');return true;
    },
    activate(found,type,value){
      const source=type==='service'?found?.serviceControl:type==='sort'?found?.sortControl:found?.directionControl;if(!source)return false;if(type==='direction'){source.click();return true;}source.value=value;source.dispatchEvent(new Event('input',{bubbles:true}));source.dispatchEvent(new Event('change',{bubbles:true}));return true;
    },
    paginator(found,host){
      host.replaceChildren();const native=found?.paginators?.[0];if(!native)return false;const small=native.querySelector('small');if(small)host.append(small.cloneNode(true));const menu=document.createElement('div');menu.className='pmf-page-controls';[...native.querySelectorAll('a[data-value],li')].forEach((item)=>{const button=document.createElement('button');button.type='button';button.textContent=item.textContent.trim();button.dataset.nativeOffset=item.dataset.value||'';button.disabled=item.matches('li,.pagination-button-disabled');if(item.matches('.pagination-button-current'))button.setAttribute('aria-current','page');menu.append(button);});host.append(menu);return true;
    },
    activatePage(found,offset){const source=[...found?.paginators?.[0]?.querySelectorAll?.('a[data-value]')||[]].find((link)=>String(link.dataset.value)===String(offset));if(!source)return false;source.click();return true;},
  };
  ...
  const CatalogueJobManager = {
    ...
    createBatch({id=`creator-batch-${Date.now()}-${++CatalogueJobManager.batchSequence}`,label='Creator batch',total=0}={}){const batch={id:String(id),label:String(label),total:Math.max(0,Number(total)||0),waiting:0,active:0,complete:0,failed:0,stopped:0,removed:0,createdAt:Date.now(),updatedAt:Date.now()};CatalogueJobManager.batches.set(batch.id,batch);CatalogueJobManager.persistSession();return batch;},
    updateBatch(batchId){if(!batchId)return null;const batch=CatalogueJobManager.batches.get(batchId)||CatalogueJobManager.createBatch({id:batchId});const jobs=[...CatalogueJobManager.pendingJobs,...CatalogueJobManager.activeJobs.values(),...CatalogueJobManager.recentJobs.values()].filter((job)=>job.batchId===batchId);const count=(states)=>jobs.filter((job)=>states.includes(job.status)).length;Object.assign(batch,{waiting:count(['queued']),active:count(['running']),complete:count(['complete']),failed:count(['failed']),stopped:count(['stopped','interrupted']),removed:Math.max(batch.removed||0,count(['removed'])),updatedAt:Date.now()});batch.total=Math.max(batch.total,jobs.length+batch.removed);return batch;},
    ...
    removeQueued(creatorKey){const job=CatalogueJobManager.queuedForCreator(creatorKey);if(!job)return false;CatalogueJobManager.pendingJobs=CatalogueJobManager.pendingJobs.filter((item)=>item!==job);CatalogueJobManager.queuedByCreator.delete(creatorKey);job.status='removed';const batch=CatalogueJobManager.batches.get(job.batchId);if(batch)batch.removed=(batch.removed||0)+1;CatalogueJobManager.updateBatch(job.batchId);CatalogueJobManager.persistSession();CatalogueJobManager.notify();return true;},
    ...
    batchCounts(batchId){const batch=CatalogueJobManager.updateBatch(batchId);if(!batch)return{total:0,active:0,waiting:0,complete:0,failed:0,stopped:0,removed:0,remaining:0};return{...batch,remaining:batch.active+batch.waiting};},
    ...
    persistSession(){const storage=globalThis.sessionStorage;if(!storage)return false;try{const serializable=(job)=>({context:job.context,requestedAction:job.requestedAction,creatorName:job.creatorName,batchId:job.batchId,batchLabel:job.batchLabel,batchSequence:job.batchSequence,queueOrder:job.queueOrder,queuedAt:job.queuedAt,progress:job.progress,status:job.status,errorMessage:job.error?.message||job.errorMessage||''});const waiting=CatalogueJobManager.pendingJobs.map(serializable);const active=[...CatalogueJobManager.activeJobs.values()].map(serializable);const recent=[...CatalogueJobManager.recentJobs.values()].map(serializable);storage.setItem(Config.creatorQueueSessionKey,JSON.stringify({version:2,waiting,active,recent,batches:[...CatalogueJobManager.batches.values()],queueOrder:CatalogueJobManager.queueOrder,updatedAt:Date.now()}));return true;}catch(error){Logger.warn('Could not persist creator queue.',error);return false;}},
    restoreSession(){if(CatalogueJobManager.sessionRestored)return false;CatalogueJobManager.sessionRestored=true;const storage=globalThis.sessionStorage;if(!storage)return false;let saved=null;try{saved=JSON.parse(storage.getItem(Config.creatorQueueSessionKey)||'null');}catch(error){Logger.warn('Could not restore creator queue.',error);}if(![1,2].includes(saved?.version))return false;CatalogueJobManager.queueOrder=Number(saved.queueOrder)||0;(saved.batches||[]).forEach((batch)=>CatalogueJobManager.batches.set(batch.id,{...batch}));[...(saved.recent||[]),...(saved.active||[]).map((job)=>({...job,status:'interrupted',progress:{...(job.progress||{}),message:'Interrupted by reload'},errorMessage:job.errorMessage||'Interrupted by reload'}))].forEach((job)=>CatalogueJobManager.recentJobs.set(job.context?.creatorKey||job.creatorKey,{...job,creatorKey:job.context?.creatorKey||job.creatorKey,error:new Error(job.errorMessage||'Interrupted by reload')}));(saved.waiting||[]).sort((a,b)=>(a.queueOrder??a.batchSequence??0)-(b.queueOrder??b.batchSequence??0)).forEach((descriptor)=>{const result=CatalogueJobManager.enqueue(descriptor.context,descriptor.requestedAction,{...descriptor});if(result.job){result.job.queueOrder=Number(descriptor.queueOrder)||result.job.queueOrder;result.job.queuedAt=Number(descriptor.queuedAt)||result.job.queuedAt;}});CatalogueJobManager.batches.forEach((_,id)=>CatalogueJobManager.updateBatch(id));CatalogueJobManager.persistSession();CatalogueJobManager.notify();return true;},
...
UI.closeSettings('reopen');const checked=(value)=>value?'checked':'';const selected=(value,current)=>value===current?'selected':'';const backdrop=document.createElement('div');backdrop.className='pmf-modal-backdrop';App.ui.settingsBadgeTypes=Util.clone(Settings.value.catalogueBadges.types);App.ui.settingsCreatorBadgeTypes=Util.clone(Settings.value.creatorCardBadges.types);App.ui.settingsStatusBadgeTypes=Util.clone(Settings.value.postStatusBadges.types);App.ui.settingsPreview={displayMode:App.displayMode,compactCardScale:Settings.value.compactCardScale,compactThumbnailAspectRatio:Settings.value.compactThumbnailAspectRatio,postAttachmentBadgeSize:Settings.value.postAttachmentBadgeSize,filteredPage:App.filteredPage,postStatusBadgeSize:Settings.value.postStatusBadgeSize,seenCardTreatment:Util.clone(Settings.value.seenCardTreatment),committed:false};
  ...
  const CreatorQueuePanel = {
    render(snapshot=CatalogueJobManager.snapshot()) {
      const host=CreatorIndexUI.queuePanel;if(!host)return;const selectedTab=host.querySelector('[data-queue-tab].pmf-active')?.dataset.queueTab||'queue';const active=snapshot.active||[],waiting=snapshot.pending||[],issues=snapshot.issues||[];const complete=(snapshot.recent||[]).filter((job)=>job.status==='complete');const batches=snapshot.batches||[];const total=batches.reduce((sum,batch)=>sum+Number(batch.total||0),0)||active.length+waiting.length+complete.length+issues.length;const completed=batches.reduce((sum,batch)=>sum+Number(batch.complete||0),0)||complete.length;const percent=total?Math.round(completed/total*100):0;
      const row=(job,kind)=>{const progress=job.progress||{};const progressValue=progress.totalPages?Math.round((Number(progress.page)||0)/progress.totalPages*100):progress.total?Math.round((Number(progress.completed)||0)/progress.total*100):0;const actions=kind==='active'?'<button data-queue-action="stop">Stop</button>':kind==='waiting'?'<button data-queue-action="top">Move to top</button><button data-queue-action="remove">Remove</button>':kind==='complete'?'': '<button data-queue-action="retry">Retry</button><button data-queue-action="dismiss">Dismiss</button>';return`<div class="pmf-queue-row" data-creator-key="${Util.escapeHtml(job.creatorKey)}"><div><strong>${Util.escapeHtml(job.creatorName||job.context?.creatorId||'Creator')}</strong><small>${Util.escapeHtml(progress.message||`${job.kind||job.requestedAction} · ${job.status}`)}</small>${kind==='active'?`<progress max="100" value="${progressValue}"></progress>`:''}</div><span>${actions}</span></div>`;};
      const batchCards=batches.map((batch)=>{const terminal=Number(batch.complete||0)+Number(batch.failed||0)+Number(batch.stopped||0)+Number(batch.removed||0),remaining=Number(batch.waiting||0)+Number(batch.active||0),batchPercent=batch.total?Math.round(terminal/batch.total*100):0;return`<section class="pmf-queue-batch" data-batch-id="${Util.escapeHtml(batch.id)}"><strong>${Util.escapeHtml(batch.label||'Creator batch')}</strong><span>${terminal} of ${Number(batch.total)||0} finished · ${remaining} remaining</span><progress max="100" value="${batchPercent}"></progress>${remaining?'<button data-queue-action="cancel-batch">Cancel remaining</button>':''}</section>`;}).join('');
      host.innerHTML=`<div class="pmf-queue-tabs"><button data-queue-tab="queue" class="pmf-active">Queue</button><button data-queue-tab="issues">Issues${issues.length?` (${issues.length})`:''}</button></div><div data-queue-pane="queue"><section class="pmf-queue-progress"><strong>Overall batch progress</strong><span>${completed} of ${total} complete · ${active.length+waiting.length} remaining · ${active.length} active</span><progress max="100" value="${percent}"></progress><small>Concurrency: ${CatalogueJobManager.concurrency}</small></section>${batchCards}${active.length?`<h3>Active</h3>${active.map((job)=>row(job,'active')).join('')}`:''}${waiting.length?`<h3>Waiting</h3>${waiting.map((job)=>row(job,'waiting')).join('')}`:''}${complete.length?`<details><summary>Recently completed (${complete.length})</summary>${complete.map((job)=>row(job,'complete')).join('')}</details><button data-queue-action="clear-completed">Clear completed</button>`:''}${!active.length&&!waiting.length&&!complete.length?'<p class="pmf-help">Queue empty</p>':''}</div><div data-queue-pane="issues" hidden>${issues.length?issues.map((job)=>row(job,'issue')).join(''):'<p class="pmf-help">No queue issues.</p>'}</div>`;
      host.querySelectorAll('[data-queue-tab]').forEach((button)=>button.classList.toggle('pmf-active',button.dataset.queueTab===selectedTab));host.querySelectorAll('[data-queue-pane]').forEach((pane)=>pane.hidden=pane.dataset.queuePane!==selectedTab);
  ...
  const CreatorIndexUI = {
    ...
    mount(found) {
      ...
      const modeSelector=document.createElement('div');modeSelector.id='pmf-creator-mode-selector';modeSelector.dataset.pmfOwned='true';modeSelector.setAttribute('role','group');modeSelector.setAttribute('aria-label','Creator directory mode');modeSelector.innerHTML='<span>Directory mode</span><button type="button" data-creator-mode="native">Native directory</button><button type="button" data-creator-mode="catalogue">Catalogue</button>';if(found.searchForm)found.searchForm.append(modeSelector);else found.searchInput?.insertAdjacentElement('afterend',modeSelector);
    ...
    sortLabel(){const item=CreatorSortUI.modes.find(([mode])=>mode===CreatorIndexUI.sort.mode);return`${item?.[1]||'Popularity'} ${CreatorIndexUI.sort.direction==='asc'?'â–²':'â–¼'}`;},
    chrome(){
      const native=CreatorIndexUI.mode==='native';CreatorIndexUI.toolbar.innerHTML=native?`<div class="pmf-controls pmf-native-proxy-controls"><select data-native-proxy="service" aria-label="Service"></select><select data-native-proxy="sort" aria-label="Native sort"></select><button class="pmf-icon-button" data-native-proxy="direction" data-creator-index-action="native-direction" aria-label="Toggle sort direction">â†•</button><span class="pmf-split-primary"><button class="pmf-scan-button" data-creator-index-action="bulk-update">Update</button><button class="pmf-scan-button pmf-split-chevron" data-creator-index-action="bulk-menu">â–¾</button></span><button class="pmf-icon-button" data-creator-index-action="settings" aria-label="Media Filter settings">${Icons.gear}</button></div>`:`<div class="pmf-controls"><button class="pmf-filter-button" data-creator-index-action="filter">All Catalogue creators</button><button class="pmf-sort-button" data-creator-index-action="sort">Sort: ${CreatorIndexUI.sortLabel()}</button><span class="pmf-split-primary"><button class="pmf-scan-button" data-creator-index-action="bulk-update">Update</button><button class="pmf-scan-button pmf-split-chevron" data-creator-index-action="bulk-menu">â–¾</button></span><button class="pmf-icon-button" data-creator-index-action="settings" aria-label="Media Filter settings">${Icons.gear}</button></div>`;
      CreatorIndexUI.toolbar.insertAdjacentHTML('beforeend','<div class="pmf-status"><div class="pmf-status-left"><strong data-creator-matches>0 matches</strong></div><div class="pmf-status-actions"><button class="pmf-details-link" data-creator-index-action="queue">Queue empty</button></div><div class="pmf-status-right" data-creator-summary></div></div><section class="pmf-catalogue-details pmf-creator-queue-panel" hidden></section>');CreatorIndexUI.queuePanel=CreatorIndexUI.toolbar.querySelector('.pmf-creator-queue-panel');CreatorIndexUI.queueButton=CreatorIndexUI.toolbar.querySelector('[data-creator-index-action="queue"]');CreatorQueuePanel.render();return !native||NativeArtistsProxy.sync(CreatorIndexUI.found,CreatorIndexUI.toolbar);
    },
    ...
    updateQueueLabel(snapshot=CatalogueJobManager.snapshot()){if(!CreatorIndexUI.queueButton)return;const issues=(snapshot.issues||[]).length;CreatorIndexUI.queueButton.textContent=snapshot.active.length||snapshot.pending.length||issues?`Queue Â· ${snapshot.active.length} active Â· ${snapshot.pending.length} waiting${issues?` Â· ${issues} issue${issues===1?'':'s'}`:''}`:'Queue empty';},
    ...
    handle(event){
      const status=event.target.closest('[data-creator-status-filter]')?.dataset.creatorStatusFilter;if(status){if(status==='native-scanned')CreatorIndexUI.nativeScannedFilter=CreatorIndexUI.nativeScannedFilter==='no-match'?'off':'no-match';else{CreatorIndexUI.statusFilters[status]=CreatorStatusFilters.cycle(CreatorIndexUI.statusFilters[status]);CreatorStatusFilters.save(CreatorIndexUI.statusFilters);}CreatorIndexUI.page=1;CreatorIndexUI.render();return;}const offset=event.target.closest('[data-native-offset]')?.dataset.nativeOffset;if(offset!==undefined&&offset!==''){NativeArtistsProxy.activatePage(CreatorIndexUI.found,offset);return;}const page=event.target.closest('[data-creator-page]')?.dataset.creatorPage;if(page){CreatorIndexUI.page=Number(page);CreatorIndexUI.render();return;}const action=event.target.closest('[data-creator-index-action]')?.dataset.creatorIndexAction;if(action==='native-direction'){NativeArtistsProxy.activate(CreatorIndexUI.found,'direction');return;}if(action==='queue'){CreatorIndexUI.queuePanel.hidden=!CreatorIndexUI.queuePanel.hidden;return;}if(action==='settings'){CreatorSettingsUI.open(event.target);return;}if(action==='filter'&&CreatorIndexUI.mode==='catalogue'){CreatorFilterUI.open(event.target);return;}if(action==='sort'&&CreatorIndexUI.mode==='catalogue'){CreatorSortUI.open(event.target);return;}if(action==='bulk-update'){CreatorBulkUI.open('update',event.target);return;}if(action==='bulk-menu'){CreatorBulkUI.openMenu(event.target);}
    },
  ...
  const CreatorBulkSelection = {
    reason(record,kind){
      const key=record.directory?.creatorKey;if(CatalogueJobManager.activeForCreator(key))return'already active';if(CatalogueJobManager.queuedForCreator(key))return'already queued';
      if(kind==='build'&&record.scanned)return'already scanned';if(kind==='resume'&&(!record.scanned||record.summary?.completeness==='complete'))return record.scanned?'Catalogue complete':'not scanned';if(kind==='update'&&!record.scanned)return'not scanned';return'';
    },
    evaluate(records,kind){const eligible=[],skipped=[];(records||[]).forEach((record)=>{const reason=CreatorBulkSelection.reason(record,kind);(reason?skipped:eligible).push(reason?{record,reason}:record);});return{eligible,skipped};},
  };
  ...
  GM_addStyle(`
    ...
    .pmf-creator-index-toolbar .pmf-native-proxy-controls{grid-template-columns:minmax(150px,1fr) minmax(190px,1fr) 52px minmax(150px,190px) 52px}
