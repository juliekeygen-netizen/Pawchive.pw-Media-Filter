...
(function () {
  ...
  const CreatorCardReconstructor = {
    ...
    buildFromTemplate(record){
      ...
      const banner=CreatorCardReconstructor.safeUrl(directory.bannerUrl);const visual=[card,...card.querySelectorAll('[style*="background-image"]')].find((node)=>node.style?.backgroundImage);if(visual){const gradient=card.dataset.pmfCreatorService==='fanbox'?'linear-gradient(rgba(67,70,74,.54),rgba(37,40,43,.88))':'linear-gradient(rgba(78,29,25,.48),rgba(45,20,18,.88))';CreatorArtworkFailureCache.applyBackground(visual,banner,gradient);}
    ...
    build(record){
      ...
      const visual=document.createElement('span');visual.className='pmf-reconstructed-creator-visual';const banner=CreatorCardReconstructor.safeUrl(directory.bannerUrl);if(banner){const gradient=card.dataset.pmfCreatorService==='fanbox'?'linear-gradient(rgba(67,70,74,.54),rgba(37,40,43,.88))':'linear-gradient(rgba(78,29,25,.48),rgba(45,20,18,.88))';CreatorArtworkFailureCache.applyBackground(visual,banner,gradient);}
...
UI.closeSettings('reopen');const checked=(value)=>value?'checked':'';const selected=(value,current)=>value===current?'selected':'';const backdrop=document.createElement('div');backdrop.className='pmf-modal-backdrop';App.ui.settingsBadgeTypes=Util.clone(Settings.value.catalogueBadges.types);App.ui.settingsCreatorBadgeTypes=Util.clone(Settings.value.creatorCardBadges.types);App.ui.settingsStatusBadgeTypes=Util.clone(Settings.value.postStatusBadges.types);App.ui.settingsPreview={displayMode:App.displayMode,compactCardScale:Settings.value.compactCardScale,compactThumbnailAspectRatio:Settings.value.compactThumbnailAspectRatio,postAttachmentBadgeSize:Settings.value.postAttachmentBadgeSize,filteredPage:App.filteredPage,postStatusBadgeSize:Settings.value.postStatusBadgeSize,seenCardTreatment:Util.clone(Settings.value.seenCardTreatment),committed:false};
  ...
  GM_addStyle(`
    ...
    .pmf-catalogue-creator-card[data-pmf-creator-service="fanbox"],.pmf-reconstructed-creator-card[data-pmf-creator-service="fanbox"]{background-color:#34383b!important}.pmf-catalogue-creator-card[data-pmf-creator-service="patreon"],.pmf-reconstructed-creator-card[data-pmf-creator-service="patreon"]{background-color:#43201d!important}
