...
(function () {
  ...
  const PopularJobManager = {
    pendingJobs:[],activeJob:null,recentJobs:new Map(),listeners:new Set(),sequence:0,restored:false,paused:false,progressNotifyTimer:null,
    ...
    notifyProgress(){if(PopularJobManager.progressNotifyTimer)return;const delay=document.visibilityState==='hidden'?750:80;PopularJobManager.progressNotifyTimer=setTimeout(()=>{PopularJobManager.progressNotifyTimer=null;PopularJobManager.notify();},delay);},
    flushProgress(){clearTimeout(PopularJobManager.progressNotifyTimer);PopularJobManager.progressNotifyTimer=null;PopularJobManager.notify();},
    ...
    suspendForBfcache(){PopularJobManager.paused=true;const active=PopularJobManager.activeJob;if(active){active.requeueAfterPause=true;active.controller?.abort();}PopularJobManager.persist();PopularJobManager.notify();},
    resumeFromBfcache(){PopularJobManager.paused=false;PopularJobManager.notify();PopularJobManager.pump();},
    shutdown(){PopularJobManager.paused=true;clearTimeout(PopularJobManager.progressNotifyTimer);PopularJobManager.progressNotifyTimer=null;PopularJobManager.persist();if(PopularJobManager.activeJob){PopularJobManager.activeJob.shutdown=true;PopularJobManager.activeJob.controller?.abort();}},
  };
  ...
  const PopularQueuePanel = {
    jobPercent(job={}){const progress=job.progress||{},total=Math.max(0,Number(progress.total)||Number(job.context?.totalPosts)||Number(job.result?.period?.totalExpectedPosts)||0),completed=Math.max(0,Number(progress.completed)||0),detailTotal=Math.max(0,Number(progress.detailTotal)||0),detailCompleted=Math.min(detailTotal,Math.max(0,Number(progress.detailCompleted)||0));if(total>0)return Util.clamp(Math.round((completed+detailCompleted)/total*100),0,100);return QueuePanelView.progress(job);},
...
sync() {
  ...
  const MaintenanceCommandRunner = {
    ...
    mode:'',interval:null,running:null,lastStartedAt:0,lastInventoryCheckAt:0,lastProfileCheckAt:0,inventoryCheckIntervalMs:5*60*1000,
