...
(function () {
  ...
  const PopularNativePaginator = {
    pending:null,
    timer:null,
    currentPage(context=App.popularContext){return Math.max(1,Math.floor(Math.max(0,Number(context?.offset)||0)/Config.pageSize)+1);},
    totalPages(total=App.dom?.totalPosts){return Math.max(1,Math.ceil(Math.max(0,Number(total)||0)/Config.pageSize));},
    target(action,page,current,total){return Util.clamp(Paginator.targetForAction(action,page,current,total),1,total);},
    renderControls(controls,context,totalPosts,host){
      if(!controls)return false;controls.replaceChildren();const current=PopularNativePaginator.currentPage(context),total=PopularNativePaginator.totalPages(totalPosts),pending=PopularNativePaginator.pending;const shown=pending?.targetPage||current;
      const add=(label,action,page,disabled,aria)=>{const button=document.createElement('button');button.type='button';button.textContent=label;button.disabled=Boolean(disabled||pending);button.dataset.popularNativePageAction=action;if(action==='page')button.dataset.popularNativePage=String(page);button.setAttribute('aria-label',aria);if(action==='page'&&page===shown){button.classList.add('pmf-current-page');button.setAttribute('aria-current','page');if(pending?.targetPage===page)button.setAttribute('aria-busy','true');}controls.append(button);};
      add('«','first',1,shown===1,'First Popular page');add('‹','previous',0,shown===1,'Previous Popular page');
      for(const page of Paginator.pageButtons(shown,total,Paginator.windowSize(Number(host?.clientWidth)||800)))add(String(page),'page',page,page===shown,`Popular page ${page}`);
      add('›','next',0,shown===total,'Next Popular page');add('»','last',total,shown===total,'Last Popular page');return true;
    },
    render(context=App.popularContext,totalPosts=App.dom?.totalPosts){PopularNativePaginator.renderControls(App.ui?.filteredControls,context,totalPosts,App.ui?.paginator);PopularNativePaginator.renderControls(App.ui?.filteredControlsBottom,context,totalPosts,App.ui?.paginatorBottom);},
    navigate(action,page,context=App.popularContext,totalPosts=App.dom?.totalPosts){
      if(!context?.date||PopularNativePaginator.pending)return false;const current=PopularNativePaginator.currentPage(context),total=PopularNativePaginator.totalPages(totalPosts),target=PopularNativePaginator.target(action,page,current,total);if(target===current)return false;
      PopularNativePaginator.pending={targetPage:target,startedAt:Date.now()};PopularNativePaginator.render(context,totalPosts);const url=PopularPeriod.pageUrl(context,(target-1)*Config.pageSize);clearTimeout(PopularNativePaginator.timer);PopularNativePaginator.timer=setTimeout(()=>PopularNativePaginator.reset({render:true}),5000);PopularNativePaginator.timer?.unref?.();
      if(globalThis.Turbo?.visit)globalThis.Turbo.visit(url);else if(typeof location.assign==='function')location.assign(url);else location.href=url;return true;
    },
    reset({render=false}={}){clearTimeout(PopularNativePaginator.timer);PopularNativePaginator.timer=null;PopularNativePaginator.pending=null;if(render&&App.pageKind==='popular'&&App.popularMode==='native')PopularNativePaginator.render();},
  };
  ...
  const PopularScanner = {
    ...
    async run(context,{signal,onProgress=()=>{},action='scan',jobId=''}={}){
      ...
      const entries=await Cache.getPopularEntries(context.periodKey);const current=entries.filter((entry)=>String(entry.runId||'')===runId);if(!current.length)throw new Error('The Popular period scan did not produce any entries.');await Cache.prunePopularEntries(context.periodKey,runId);const finalStatus=detailFailed?'partial':'complete';const final={...existing,...base,status:finalStatus,workingRunId:detailFailed?runId:'',completedRunId:detailFailed?String(existing?.completedRunId||''):runId,coveredOffsets:[...covered],totalExpectedPosts:total,storedEntryCount:current.length,retryableCount:detailFailed,retryPostKeys:[...failedPostKeys],lastScanAt:action==='scan'||action==='resume'?Date.now():Number(existing?.lastScanAt)||Date.now(),lastUpdateAt:Date.now(),completedAt:Date.now(),updatedAt:Date.now()};await Cache.putPopularPeriod(final);onProgress({page:pagesScanned,totalPages:offsets.length,completed:current.length,total:current.length,message:detailFailed?`Saved ${current.length} posts · ${detailFailed} metadata requests need retry`:`Saved ${current.length} Popular posts`});return{period:final,entries:current,newCount:current.length,detailFailed};
    },
  };
  ...
  const PopularJobManager = {
    ...
    restore(){if(PopularJobManager.restored)return;PopularJobManager.restored=true;PopularJobManager.paused=false;try{const raw=globalThis.sessionStorage?.getItem(Config.popularQueueSessionKey);const data=raw?JSON.parse(raw):null;const descriptors=data?.descriptors||[];PopularJobManager.sequence=Math.max(PopularJobManager.sequence,...descriptors.map((item)=>Math.max(0,Number(item?.queueOrder)||0)),0);for(const descriptor of descriptors){if(!descriptor?.context?.periodKey)continue;PopularJobManager.pendingJobs.push({id:`popular-job-${Date.now()}-${++PopularJobManager.sequence}`,periodKey:descriptor.context.periodKey,context:descriptor.context,requestedAction:descriptor.status==='interrupted'?'resume':descriptor.requestedAction||'resume',status:'queued',queuedAt:descriptor.queuedAt||Date.now(),queueOrder:descriptor.queueOrder||PopularJobManager.sequence,progress:{message:'Restored after reload'}});}PopularJobManager.pendingJobs.sort((a,b)=>a.queueOrder-b.queueOrder);PopularJobManager.pump();}catch(error){Logger.warn('Popular queue session could not be restored.',error);}},
...
UI.closeSettings('reopen');const checked=(value)=>value?'checked':'';const selected=(value,current)=>value===current?'selected':'';const backdrop=document.createElement('div');backdrop.className='pmf-modal-backdrop';App.ui.settingsBadgeTypes=Util.clone(Settings.value.catalogueBadges.types);App.ui.settingsCreatorBadgeTypes=Util.clone(Settings.value.creatorCardBadges.types);App.ui.settingsStatusBadgeTypes=Util.clone(Settings.value.postStatusBadges.types);App.ui.settingsPreview={displayMode:App.displayMode,compactCardScale:Settings.value.compactCardScale,compactThumbnailAspectRatio:Settings.value.compactThumbnailAspectRatio,postAttachmentBadgeSize:Settings.value.postAttachmentBadgeSize,filteredPage:App.filteredPage,postStatusBadgeSize:Settings.value.postStatusBadgeSize,seenCardTreatment:Util.clone(Settings.value.seenCardTreatment),committed:false};
  ...
  const PopularCardDecorator = {
    apply(card,post){
      card.querySelector?.('[data-pmf-popular-metric]')?.remove();
      const entry=App.popularEntries.get(String(post?.key||''));if(!entry)return card;card.dataset.pmfPostKey=String(post.key||'');
      const favorites=Number(entry.displayedFavoriteCount),footer=card.querySelector?.('.post-card__footer');if(footer){const leaves=[...footer.querySelectorAll?.('div,span')||[]].filter((node)=>!/\S/.test([...node.children||[]].map((child)=>child.textContent||'').join('')));const favoriteNode=leaves.find((node)=>/\bfavou?rites?\b/i.test(node.textContent||''));if(Number.isFinite(favorites)){if(favoriteNode)favoriteNode.textContent=`${favorites.toLocaleString()} favorites`;else{const line=document.createElement('div');line.dataset.pmfPopularFavorites='true';line.textContent=`${favorites.toLocaleString()} favorites`;footer.append(line);}}else favoriteNode?.remove();}
      const metric=document.createElement('div');metric.dataset.pmfPopularMetric='true';metric.className='pmf-popular-card-metric';metric.textContent=`#${Math.max(1,Number(entry.rank)||1)}${Number.isFinite(favorites)?` · ${favorites.toLocaleString()} favorites`:''}`;
      card.append(metric);return card;
    },
  };

  const PopularPageController = {
    root:null,found:null,page:null,context:null,controller:null,unsubscribe:null,observer:null,nativeSnapshot:null,lastCompletedAt:0,queueOpen:false,refreshRevision:0,observerSchedule:null,rebinding:false,
    ...
    periodLinkDescriptor(link,context){try{const url=new URL(link.href||link.getAttribute?.('href'),location.origin);const label=String(link.textContent||'').trim().replace(/\s+/g,' ');const rel=String(link.getAttribute?.('rel')||'').toLowerCase();let role='';if(rel==='prev'||/previous|prev|[‹«←]/i.test(label))role='previous';else if(rel==='next'||/next|[›»→]/i.test(label))role='next';else role=PopularPeriod.normalizePeriod(url.searchParams.get('period'));return{href:url.href,label:label||({previous:'Previous',next:'Next',day:'Day',week:'Week',month:'Month'}[role]||'Period'),role};}catch{return null;}},
    periodNavMarkup(found,context){
      const descriptors=(found.periodLinks||[]).map((link)=>PopularPageController.periodLinkDescriptor(link,context)).filter(Boolean);const byRole=new Map();for(const descriptor of descriptors){if(!byRole.has(descriptor.role)||['previous','next'].includes(descriptor.role))byRole.set(descriptor.role,descriptor);}
      for(const period of ['day','week','month'])if(!byRole.has(period))byRole.set(period,{role:period,href:`${location.origin}/posts/popular?date=${encodeURIComponent(context.date)}&period=${period}`,label:period[0].toUpperCase()+period.slice(1)});
      const values=['previous','day','week','month','next'].map((role)=>byRole.get(role)).filter(Boolean);return `<div class="pmf-popular-period-title">${Util.escapeHtml(context.label)}</div><nav class="pmf-popular-period-links" aria-label="Popular Posts period navigation">${values.map(({href,label,role})=>`<a href="${Util.escapeHtml(href)}" data-popular-period-role="${role}" ${role===context.period?'aria-current="page"':''}>${Util.escapeHtml(label)}</a>`).join('')}</nav>`;
    },
    async loadStatuses(context=App.popularContext,entries=[...App.popularEntries.values()]){if(!context)return;const statuses=await Cache.getAllPostStatuses(),relevant=new Set((entries||[]).map((entry)=>String(entry.postKey||'')));App.statuses=new Map();for(const raw of statuses){const status=PostStatus.normalize(raw);if(relevant.has(status.key)){App.statuses.set(status.key,status);App.statuses.set(String(status.postId),status);}}App.favoriteSnapshotMeta=await Cache.getFavoriteSyncMeta(context.domain);App.favoriteSnapshotMembership=await Cache.getFavoriteSnapshotKeys(context.domain,App.favoriteSnapshotMeta?.activeSnapshotId);},
    async load(context,found,{signal}={}){
      ...
      const globalMode=GM_getValue(Config.popularModeKey,'native');App.popularMode=['native','local'].includes(saved?.mode)?saved.mode:['native','local'].includes(globalMode)?globalMode:'native';PopularPageController.assertMountCurrent(context,signal);
    },
    mountUI(found,context){
      Lifecycle.removeStaleRoots();const root=UI.own(document.createElement('section'));root.id='pmf-popular-root';root.className='pmf-popular-root';root.dataset.popularMode=App.popularMode;root.innerHTML=`<div class="pmf-popular-period pmf-surface">${PopularPageController.periodNavMarkup(found,context)}</div><div class="pmf-popular-mode" role="group" aria-label="Popular Posts mode"><button type="button" data-popular-mode="native" class="${App.popularMode==='native'?'pmf-active':''}" aria-pressed="${App.popularMode==='native'}">Native</button><button type="button" data-popular-mode="local" class="${App.popularMode==='local'?'pmf-active':''}" aria-pressed="${App.popularMode==='local'}">Local</button></div><section class="pmf-toolbar pmf-popular-toolbar"><div class="pmf-controls"><button type="button" class="pmf-filter-button"></button><button type="button" class="pmf-sort-button pmf-locked-control" disabled>Sort: Popular</button><button type="button" class="pmf-scan-button"></button><button type="button" class="pmf-icon-button" aria-label="Media Filter settings">${Icons.gear}</button></div><div class="pmf-status"><div class="pmf-status-left"><button type="button" class="pmf-link-button" data-popular-queue-toggle>Queue empty</button><strong data-popular-status-label></strong></div><div class="pmf-status-actions"></div><div class="pmf-status-right"></div></div><div class="pmf-progress" hidden><span></span></div><div class="pmf-popular-queue" hidden></div></section>`;
      const anchor=found.navContainer||found.heading||found.grid;anchor.insertAdjacentElement('afterend',root);
      const paginator=UI.own(document.createElement('div'));paginator.className='pmf-filtered-paginator pmf-popular-paginator';const statusFilters=document.createElement('div');statusFilters.className='pmf-quick-status-filters';for(const field of ['favorite','liked','seen']){const button=document.createElement('button');button.type='button';button.dataset.pmfQuickStatus=field;button.innerHTML=`<span class="pmf-quick-status-main">${PostStatus.icon(field)}</span><span class="pmf-quick-status-negate">${Icons.x}</span>`;statusFilters.append(button);}const filteredCount=document.createElement('small');filteredCount.className='pmf-filtered-count';const filteredControls=document.createElement('div');filteredControls.className='pmf-page-controls';paginator.append(statusFilters,filteredCount,filteredControls);found.grid.insertAdjacentElement('beforebegin',paginator);
      const grid=UI.own(document.createElement('div'));grid.className=[found.grid.className,'pmf-filter-grid','pmf-popular-local-grid'].filter(Boolean).join(' ');grid.hidden=true;found.grid.insertAdjacentElement('afterend',grid);const paginatorBottom=UI.own(document.createElement('div'));paginatorBottom.className='pmf-filtered-paginator pmf-bottom-paginator pmf-popular-paginator';const filteredCountBottom=document.createElement('small');filteredCountBottom.className='pmf-filtered-count';const filteredControlsBottom=document.createElement('div');filteredControlsBottom.className='pmf-page-controls';paginatorBottom.append(filteredCountBottom,filteredControlsBottom);grid.insertAdjacentElement('afterend',paginatorBottom);
      App.dom={...found,countNodes:[],menuNodes:[],topPaginator:found.topPaginator,bottomPaginator:found.bottomPaginator};App.ui={root,toolbar:root.querySelector('.pmf-popular-toolbar'),controls:root.querySelector('.pmf-controls'),filterButton:root.querySelector('.pmf-filter-button'),sortButton:root.querySelector('.pmf-sort-button'),scanButton:root.querySelector('.pmf-scan-button'),settingsButton:root.querySelector('.pmf-icon-button'),statusLeft:root.querySelector('.pmf-status-left'),statusLabel:root.querySelector('[data-popular-status-label]'),statusActions:root.querySelector('.pmf-status-actions'),statusRight:root.querySelector('.pmf-status-right'),progress:root.querySelector('.pmf-progress'),progressBar:root.querySelector('.pmf-progress span'),details:document.createElement('div'),paginator,filteredCount,statusFilters,filteredControls,paginatorBottom,filteredCountBottom,filteredControlsBottom,grid,filterPopover:null,editor:null,settings:null,sortMenu:null,queue:root.querySelector('.pmf-popular-queue')};
      PopularPageController.root=root;PopularPageController.bind();UI.updateQuickStatusFilters();CompactLayoutEngine.connect();
    },
    bind(){const signal=PopularPageController.controller.signal,ui=App.ui;
      ui.root.addEventListener('click',(event)=>{const mode=event.target.closest('[data-popular-mode]')?.dataset.popularMode;if(mode){App.popularMode=mode;GM_setValue(Config.popularModeKey,mode);App.filteredPage=1;App.persistUIState();App.render();return;}if(event.target.closest('.pmf-filter-button')&&App.popularMode==='local')UI.toggleFilterPopover();if(event.target.closest('.pmf-scan-button'))PopularPageController.primaryAction();if(event.target.closest('.pmf-icon-button'))UI.openSettings();if(event.target.closest('[data-popular-queue-toggle]')){PopularPageController.queueOpen=!PopularPageController.queueOpen;PopularPageController.renderQueue();}const action=event.target.closest('[data-popular-job-action]');if(action){const key=action.dataset.periodKey;if(action.dataset.popularJobAction==='stop')PopularJobManager.stop(key);if(action.dataset.popularJobAction==='remove')PopularJobManager.remove(key);if(action.dataset.popularJobAction==='retry')PopularJobManager.retry(key);if(action.dataset.popularJobAction==='top')PopularJobManager.moveToTop(key);if(action.dataset.popularJobAction==='clear')PopularJobManager.clearCompleted();}}, {signal});
      const pageClick=(event)=>{const native=event.target.closest('button[data-popular-native-page-action]');if(native&&!native.disabled){PopularNativePaginator.navigate(native.dataset.popularNativePageAction,native.dataset.popularNativePage);return;}const button=event.target.closest('button[data-pmf-page-action]');if(button&&!button.disabled)Paginator.activate(button.dataset.pmfPageAction,button.dataset.pmfPage);};ui.paginator.addEventListener('click',pageClick,{signal});ui.paginatorBottom.addEventListener('click',pageClick,{signal});
      ...
      document.addEventListener('keydown',(event)=>{if(!['ArrowLeft','ArrowRight'].includes(event.key)||OverlayManager.top()||event.target?.closest?.('input,textarea,select,button,[contenteditable="true"]'))return;const role=event.key==='ArrowRight'?'next':'previous';const changed=App.popularMode==='local'?Paginator.activate(role):PopularNativePaginator.navigate(role);if(changed)event.preventDefault();},{capture:true,signal});
    },
    actionFor(meta=App.popularMeta){const stored=Math.max(0,Number(meta?.storedEntryCount)||0);if(!meta||(!stored&&!meta.workingRunId))return'scan';if(meta.status==='partial'||meta.status==='building'||meta.status==='updating'||meta.workingRunId)return'resume';return'update';},
    async primaryAction(){const context=App.popularContext;if(!context)return;const active=PopularJobManager.activeJob;if(active?.periodKey===context.periodKey){PopularJobManager.stop(context.periodKey);return;}const queued=PopularJobManager.pendingJobs.find((job)=>job.periodKey===context.periodKey);if(queued){PopularJobManager.remove(context.periodKey);UI.flash('Popular period removed from queue.');return;}const action=PopularPageController.actionFor();const total=Math.max(1,Number(App.dom?.totalPosts)||Number(App.popularMeta?.totalExpectedPosts)||500);const label=action==='scan'?'Queue scan':action==='resume'?'Queue resume':'Queue update';UI.confirmDialog({title:`${action==='scan'?'Scan':action==='resume'?'Resume':'Update'} Popular Posts?`,paragraphs:[`Period: ${context.label}`,`Posts: up to ${total.toLocaleString()}`,'This stores post metadata, native rank, and the displayed favorite count. Media files are not downloaded.','Posts shown on Popular pages are marked as having no missing attachments at this observation time.'],confirmLabel:label,opener:App.ui.scanButton,onConfirm:()=>{const result=PopularJobManager.enqueue({...context,totalPosts:total,totalKnown:Boolean(Number(App.dom?.totalPosts)||Number(App.popularMeta?.totalExpectedPosts))},action);if(!result.accepted)UI.flash(result.state==='already-queued'?`Already queued · Position ${result.position}`:'This Popular period is already running.');App.render();}});},
    renderQueue(){const ui=App.ui;if(!ui)return;const snapshot=PopularJobManager.snapshot(),jobs=[...snapshot.active,...snapshot.pending,...snapshot.recent];ui.queue.hidden=!PopularPageController.queueOpen;const toggle=ui.root.querySelector('[data-popular-queue-toggle]');if(toggle)toggle.textContent=snapshot.active.length||snapshot.pending.length?`${snapshot.active.length} active · ${snapshot.pending.length} waiting`:snapshot.recent.length?`${snapshot.recent.length} recent job${snapshot.recent.length===1?'':'s'}`:'Queue empty';if(!PopularPageController.queueOpen)return;ui.queue.innerHTML=jobs.length?jobs.map((job)=>{const progress=job.progress||{},action=job.status==='running'?'stop':job.status==='queued'?'remove':['failed','stopped','interrupted'].includes(job.status)?'retry':'';return`<div class="pmf-popular-job"><div><strong>${Util.escapeHtml(PopularPeriod.label(job.context))}</strong><span>${Util.escapeHtml(progress.message||job.status)}</span></div><div>${job.status==='queued'?`<button data-popular-job-action="top" data-period-key="${Util.escapeHtml(job.periodKey)}">Top</button>`:''}${action?`<button data-popular-job-action="${action}" data-period-key="${Util.escapeHtml(job.periodKey)}">${action==='stop'?'Stop':action==='remove'?'Remove':'Retry'}</button>`:''}</div></div>`;}).join(''):`<div class="pmf-empty-inline">Queue empty</div>`;},
    renderAction(){const job=PopularJobManager.jobForPeriod(App.popularContext?.periodKey),action=PopularPageController.actionFor(),nativeRequired=App.popularMode==='local'&&action==='scan'&&!job;App.ui.scanButton.disabled=nativeRequired;App.ui.scanButton.textContent=job?.status==='running'?'Stop':job?.status==='queued'?`Queued · ${PopularJobManager.queuePosition(job.periodKey)}`:nativeRequired?'Scan in Native':action==='scan'?'Scan':action==='resume'?'Resume':'Update';App.ui.scanButton.classList.toggle('pmf-stop-button',Boolean(job&&['running','queued'].includes(job.status)));App.ui.progress.hidden=job?.status!=='running';if(job?.status==='running'){const completed=Number(job.progress?.completed)||Number(job.progress?.detailCompleted)||0,total=Number(job.progress?.total)||Number(job.progress?.detailTotal)||App.totalPosts||1;App.ui.progressBar.style.width=`${Math.min(100,completed/Math.max(1,total)*100)}%`;}},
    applyNativeControlVisibility(found=App.dom){for(const node of found?.navContainers||[])if(PopularDOM.isSafeNativeControlGroup(node,found.grid))PopularPageController.setVisible(node,false);for(const node of found?.paginators||[])PopularPageController.setVisible(node,false);},
    renderNative(){
      PopularPageController.setVisible(App.dom.grid,true);PopularPageController.setVisible(App.ui.grid,false);PopularPageController.applyNativeControlVisibility();App.ui.statusFilters.hidden=true;App.ui.filterButton.textContent='All posts';App.ui.filterButton.disabled=true;App.ui.filterButton.classList.add('pmf-locked-control');App.ui.sortButton.textContent='Sort: Popular';App.ui.sortButton.disabled=true;App.ui.sortButton.classList.add('pmf-locked-control');App.ui.paginator.hidden=false;App.ui.paginatorBottom.hidden=false;const total=App.dom.totalPosts||App.dom.nativeCards.length;const current=PopularNativePaginator.currentPage();const start=total?(current-1)*Config.pageSize+1:0,end=Math.min(total,current*Config.pageSize);App.ui.filteredCount.textContent=total?`Showing ${start}–${end} of ${total}`:'Showing native Popular Posts';App.ui.filteredCountBottom.textContent=App.ui.filteredCount.textContent;PopularNativePaginator.render(App.popularContext,total);App.ui.statusLabel.textContent='Native Popular Posts';App.ui.statusRight.textContent=App.popularMeta?`Local snapshot · ${App.popularMeta.storedEntryCount||0} posts`:'Ready to scan this period';
    },
    renderLocal(){
      PopularPageController.setVisible(App.dom.grid,false);PopularPageController.applyNativeControlVisibility();PopularPageController.setVisible(App.ui.grid,true);App.ui.statusFilters.hidden=false;App.ui.filterButton.disabled=false;App.ui.filterButton.classList.remove('pmf-locked-control');UI.updateFilterButton();UI.updateQuickStatusFilters();App.ui.sortButton.textContent='Sort: Popular';App.ui.sortButton.disabled=true;App.ui.sortButton.classList.add('pmf-locked-control');
      const matches=App.matchingPosts();App.ui.paginator.hidden=false;App.ui.paginatorBottom.hidden=false;CompactLayoutEngine.apply({reason:'popular-render',verify:false});if(App.filteredAnchorId)App.restoreAnchorPage();else App.restoreFirstResultPage(matches.length);Paginator.render(matches.length);const size=App.filteredPageSize(),start=(App.filteredPage-1)*size;App.filteredFirstResultIndex=start;App.filteredAnchorId=App.postAnchorKey(matches[start])||App.filteredAnchorId;App.ui.grid.replaceChildren();const pagePosts=matches.slice(start,start+size);
      if(!App.popularMeta){const empty=document.createElement('div');empty.className='pmf-empty';empty.innerHTML='<strong>This Popular Posts period has not been scanned.</strong><span>Switch to Native and queue a scan.</span>';App.ui.grid.append(empty);}else if(!pagePosts.length){const empty=document.createElement('div');empty.className='pmf-empty';empty.innerHTML='<strong>No matching posts</strong><span>Adjust the media or status filters.</span>';App.ui.grid.append(empty);}else for(const post of pagePosts){const card=App.dom.template?CardRenderer.clone(post):CardRenderer.fallback(post);PopularCardDecorator.apply(card,post);App.ui.grid.append(card);}CompactLayoutEngine.apply({reason:'popular-render'});App.ui.statusLabel.textContent=`${matches.length} matches`;App.ui.statusRight.textContent=`Local · ${App.popularEntries.size} stored · Sort: Popular`;
    },
    ...
    rebindNative(found,page=Route.parsePage(location.href)){
      if(PopularPageController.rebinding||!found?.grid||page.kind!=='popular')return false;const context=PopularPeriod.fromPage(page,found);if(!context.periodKey||context.pageKey!==App.popularContext?.pageKey)return false;PopularPageController.rebinding=true;try{PopularPageController.restoreNative();PopularPageController.nativeSnapshot=PopularPageController.saveNative(found);PopularPageController.found=found;App.dom={...App.dom,...found,topPaginator:found.topPaginator,bottomPaginator:found.bottomPaginator};App.dom.template=found.template||App.dom.template;App.dom.totalPosts=found.totalPosts||App.dom.totalPosts;const periodHost=App.ui?.root?.querySelector?.('.pmf-popular-period');if(periodHost)periodHost.innerHTML=PopularPageController.periodNavMarkup(found,context);PopularNativePaginator.reset();PopularPageController.applyNativeControlVisibility(found);App.render();return true;}finally{PopularPageController.rebinding=false;}
    },
  ...
  GM_addStyle(`
    ...
    #pmf-popular-root{position:relative;z-index:32;width:min(960px,calc(100% - 24px));margin:8px auto 10px;color:var(--pmf-text)}
    .pmf-popular-period{padding:8px 12px;margin:0 auto 8px;text-align:center;background:linear-gradient(180deg,#1f2326e8,#171a1de8)}
    .pmf-popular-period-title{margin-bottom:5px;color:#f1d0c0;font-size:13px;font-weight:650}
    .pmf-popular-period-links{display:flex;flex-wrap:wrap;justify-content:center;gap:4px 9px}
    .pmf-popular-period-links a{color:#efaaa4;text-decoration:none;font-size:12px;white-space:nowrap}
    .pmf-popular-period-links a:hover{text-decoration:underline}.pmf-popular-period-links a[aria-current="page"]{color:#ffd0b4;font-weight:700;text-decoration:underline}.pmf-popular-period-links a[data-popular-period-role="previous"],.pmf-popular-period-links a[data-popular-period-role="next"]{color:#f4c6b0}
    .pmf-popular-mode{display:grid;grid-template-columns:1fr 1fr;width:min(320px,calc(100% - 16px));margin:0 auto 8px}
    .pmf-popular-mode button{min-height:32px;color:var(--pmf-text);background:var(--pmf-surface-1);border:1px solid var(--pmf-border);cursor:pointer}
    ...
    .pmf-popular-toolbar .pmf-controls{grid-template-columns:minmax(210px,1fr) minmax(170px,.85fr) minmax(120px,.6fr) 42px}
    ...
    .pmf-popular-toolbar .pmf-status-left{display:flex;align-items:center;gap:10px;min-width:0}.pmf-popular-toolbar .pmf-status-left strong{overflow:hidden;text-overflow:ellipsis;white-space:nowrap}.pmf-popular-toolbar .pmf-status-actions{margin-left:auto}
    ...
    .pmf-popular-queue{max-height:280px;overflow:auto;border-top:1px solid var(--pmf-border);padding:7px 9px;background:#15181b}
    .pmf-popular-job{display:grid;grid-template-columns:minmax(0,1fr) auto;gap:10px;align-items:center;padding:7px 0;border-bottom:1px solid #32383d}
    .pmf-popular-job:last-child{border-bottom:0}.pmf-popular-job>div:first-child{display:flex;flex-direction:column;min-width:0}.pmf-popular-job strong{overflow:hidden;text-overflow:ellipsis;white-space:nowrap}.pmf-popular-job span{color:var(--pmf-muted);font-size:11px}.pmf-popular-job>div:last-child{display:flex;gap:5px}.pmf-popular-job button{min-height:28px;padding:0 8px}
    .pmf-popular-card-metric{position:absolute;left:4px;bottom:4px;z-index:8;max-width:calc(100% - 8px);padding:2px 5px;border:1px solid #d6946f;border-radius:3px;background:#111d;color:#ffd0b4;font:700 10px/1.2 var(--pmf-font);white-space:nowrap;overflow:hidden;text-overflow:ellipsis;pointer-events:none}
    .pmf-popular-local-grid>.post-card .post-card__footer{padding-bottom:21px!important}
    .pmf-empty-inline{padding:8px;color:var(--pmf-muted);text-align:center}
    @media(max-width:760px){
      #pmf-popular-root{width:calc(100% - 10px)}
      .pmf-popular-toolbar .pmf-controls{grid-template-columns:minmax(0,1fr) minmax(120px,150px) 46px;gap:7px}
      .pmf-popular-toolbar .pmf-filter-button{grid-column:1/-1}.pmf-popular-toolbar .pmf-sort-button{grid-column:1}.pmf-popular-toolbar .pmf-scan-button{grid-column:2}.pmf-popular-toolbar .pmf-icon-button{grid-column:3}
      .pmf-popular-period-links{gap:6px 12px}.pmf-popular-period-links a{font-size:13px}
      .pmf-popular-job{grid-template-columns:1fr}.pmf-popular-job>div:last-child{justify-content:flex-end}
      .pmf-popular-queue{max-height:38dvh}
    }
  `);
  ...
  const Lifecycle = {
    ...
    async performEnsureMounted(requestedPage,{generation,signal,priorGrid,priorSignatureKey}={}) {
      ...
      if(page.kind==='popular'){PostPageController.cleanup();CreatorPageController.cleanup();ArtistsPageController.cleanup();Lifecycle.removeStaleRoots();await PopularPageController.mount(page,{generation,signal});return;}
