...
UI.closeSettings('reopen');const checked=(value)=>value?'checked':'';const selected=(value,current)=>value===current?'selected':'';const backdrop=document.createElement('div');backdrop.className='pmf-modal-backdrop';App.ui.settingsBadgeTypes=Util.clone(Settings.value.catalogueBadges.types);App.ui.settingsCreatorBadgeTypes=Util.clone(Settings.value.creatorCardBadges.types);App.ui.settingsStatusBadgeTypes=Util.clone(Settings.value.postStatusBadges.types);App.ui.settingsPreview={displayMode:App.displayMode,compactCardScale:Settings.value.compactCardScale,compactThumbnailAspectRatio:Settings.value.compactThumbnailAspectRatio,postAttachmentBadgeSize:Settings.value.postAttachmentBadgeSize,filteredPage:App.filteredPage,postStatusBadgeSize:Settings.value.postStatusBadgeSize,seenCardTreatment:Util.clone(Settings.value.seenCardTreatment),committed:false};
  ...
  const PopularPageController = {
    ...
    saveNative(found){return{grid:PopularPageController.nativeState(found.grid),paginators:(found.paginators||[]).map(PopularPageController.nativeState),counts:(found.countNodes||[]).map(PopularPageController.nativeState),navs:(found.navContainers||[]).filter((node)=>PopularDOM.isSafeNativeControlGroup(node,found.grid)).map(PopularPageController.nativeState),heading:found.heading?{node:found.heading,text:String(found.heading.textContent||'')}:null};},
    ...
    bind(){const signal=PopularPageController.controller.signal,ui=App.ui;PopularPageController.layoutSchedule=Util.debounce(()=>{if(PopularPageController.mounted()&&App.popularMode!=='native')PopularPageController.applyLocalLayout();},120);window.addEventListener('resize',PopularPageController.layoutSchedule,{signal});signal.addEventListener('abort',()=>PopularPageController.layoutSchedule?.cancel?.(),{once:true});
      ...
      ui.root.addEventListener('click',(event)=>{if(event.target.closest('[data-queue-action],[data-queue-tab]')){PopularQueuePanel.handle(event);return;}const mode=event.target.closest('[data-popular-mode]')?.dataset.popularMode;if(mode){PopularPageController.setMode(mode);return;}}, {signal});
      document.addEventListener('click',(event)=>{const link=event.target?.closest?.('a[href]');if(!link||!(App.dom?.periodLinks||[]).includes(link))return;const url=PopularNavigation.normalize(link.getAttribute?.('href')||link.href||'');if(!url||url.searchParams.has('o'))return;if(App.popularMode==='aggregate'){const period=link.dataset.pmfAggregatePeriod||(/^(?:day|week|month)$/i.test(String(link.textContent||'').trim())?PopularPeriod.normalizePeriod(url.searchParams.get('period')):'');if(!period)return;event.preventDefault();event.stopPropagation();PopularPageController.setAggregatePeriod(period);return;}if(url.href===location.href)return;event.preventDefault();GM_setValue(Config.popularModeKey,App.popularMode);PopularNavigation.visit(url.href,{reason:'popular-native-period'});},{capture:true,signal});
    ...
    restoreAggregatePeriodControls(){for(const[link,state]of PopularPageController.aggregateLinkStates){if(!link?.isConnected)continue;link.hidden=state.hidden;if(state.display)link.style?.setProperty?.('display',state.display,state.priority||'');else link.style?.removeProperty?.('display');if(state.aria==null)link.removeAttribute?.('aria-hidden');else link.setAttribute?.('aria-hidden',state.aria);link.classList?.remove?.('pmf-popular-aggregate-period-active');delete link.dataset.pmfAggregatePeriod;}PopularPageController.aggregateLinkStates.clear();for(const node of App.dom?.navContainers||[])node.classList?.remove?.('pmf-popular-aggregate-periods');const heading=PopularPageController.nativeSnapshot?.heading;if(heading?.node?.isConnected)heading.node.textContent=heading.text;},
    syncAggregatePeriodControls(found=App.dom){if(App.popularMode!=='aggregate'){PopularPageController.restoreAggregatePeriodControls();return;}for(const container of found?.navContainers||[])container.classList?.add?.('pmf-popular-aggregate-periods');for(const link of found?.periodLinks||[]){let url;try{url=new URL(link.getAttribute?.('href')||link.href||'',location.origin);}catch{continue;}const label=String(link.textContent||link.getAttribute?.('aria-label')||'').replace(/\s+/g,' ').trim(),exact=/^(day|week|month)$/i.exec(label),period=exact?PopularPeriod.normalizePeriod(url.searchParams.get('period')||exact[1]):'';PopularPageController.rememberAggregateLink(link);if(!period){link.hidden=true;link.style?.setProperty?.('display','none','important');link.setAttribute?.('aria-hidden','true');delete link.dataset.pmfAggregatePeriod;continue;}link.hidden=false;link.style?.removeProperty?.('display');link.removeAttribute?.('aria-hidden');link.dataset.pmfAggregatePeriod=period;link.classList?.toggle?.('pmf-popular-aggregate-period-active',period===App.popularAggregatePeriod);}const heading=PopularPageController.nativeSnapshot?.heading;if(heading?.node?.isConnected)heading.node.textContent=`All Scanned Popular Posts · ${App.popularAggregatePeriod[0].toUpperCase()}${App.popularAggregatePeriod.slice(1)}`;},
  ...
  GM_addStyle(`
    ...
    .pmf-popular-aggregate-periods{display:flex!important;align-items:center;justify-content:center;gap:14px!important;line-height:1.25!important}.pmf-popular-aggregate-periods a[data-pmf-aggregate-period]{display:inline-block!important;padding:1px 3px;color:#e6b096!important;text-decoration:none!important}.pmf-popular-aggregate-periods a.pmf-popular-aggregate-period-active{color:#ffd1b7!important;font-weight:700;text-decoration:underline!important;text-underline-offset:3px}
